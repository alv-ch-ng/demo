/* alv-ch-ng.demo - 1.0.0 - 2014-12 - 12 - Copyright (c) 2014 ICT ALV-CH; */
...
}),hljs.registerLanguage("xl",function(e){var t="ObjectLoader Animate MovieCredits Slides Filters Shading Materials LensFlare Mapping VLCAudioVideo StereoDecoder PointCloud NetworkAccess RemoteControl RegExp ChromaKey Snowfall NodeJS Speech Charts",r={keyword:"if then else do while until for loop import with is as where when by data constant",literal:"true false nil",type:"integer real text name boolean symbol infix prefix postfix block tree",built_in:"in mod rem and or xor not abs sign floor ceil sqrt sin cos tan asin acos atan exp expm1 log log2 log10 log1p pi at",module:t,id:"text_length text_range text_find text_replace contains page slide basic_slide title_slide title subtitle fade_in fade_out fade_at clear_color color line_color line_width texture_wrap texture_transform texture scale_?x scale_?y scale_?z? translate_?x translate_?y translate_?z? rotate_?x rotate_?y rotate_?z? rectangle circle ellipse sphere path line_to move_to quad_to curve_to theme background contents locally time mouse_?x mouse_?y mouse_buttons"},a={cN:"constant",b:"[A-Z][A-Z_0-9]+",r:0},i={cN:"variable",b:"([A-Z][a-z_0-9]+)+",r:0},n={cN:"id",b:"[a-z][a-z_0-9]+",r:0},o={cN:"string",b:'"',e:'"',i:"\\n"},s={cN:"string",b:"'",e:"'",i:"\\n"},l={cN:"string",b:"<<",e:">>"},c={cN:"number",b:"[0-9]+#[0-9A-Z_]+(\\.[0-9-A-Z_]+)?#?([Ee][+-]?[0-9]+)?",r:10},p={cN:"import",bK:"import",e:"$",k:{keyword:"import",module:t},r:0,c:[o]},d={cN:"function",b:"[a-z].*->"};return{aliases:["tao"],l:/[a-zA-Z][a-zA-Z0-9_?]*/,k:r,c:[e.CLCM,e.CBCM,o,s,l,d,p,a,i,n,c,e.NM]}}),function(){"use strict";function checkAndHandlePermission(SecurityService,element,checkFn,itemString,scope){if(itemString){var tokens=itemString.replace(matcher,"").split(",");checkFn(tokens)||element.addClass("ng-hide"),scope.$on(SecurityService.getConstants().EVENT_NAME_LOGOUT,function(){checkFn(tokens)?element.removeClass("ng-hide"):element.addClass("ng-hide")}),scope.$on(SecurityService.getConstants().EVENT_NAME_LOGIN_CONFIRMED,function(){checkFn(tokens)&&element.removeClass("ng-hide")})}}function checkAndHandleAuth(SecurityService,scope,element,inverseOrder){SecurityService.isAuthenticated()?inverseOrder?element.addClass("ng-hide"):element.removeClass("ng-hide"):inverseOrder?element.removeClass("ng-hide"):element.addClass("ng-hide"),scope.$on(SecurityService.getConstants().EVENT_NAME_LOGIN_CONFIRMED,function(){inverseOrder?element.addClass("ng-hide"):element.removeClass("ng-hide")}),scope.$on(SecurityService.getConstants().EVENT_NAME_LOGOUT,function(){inverseOrder?element.removeClass("ng-hide"):element.addClass("ng-hide")})}var module=angular.module("alv-ch-ng.security",["alv-ch-ng.ui-forms"]);module.provider("AuthenticationService",function($httpProvider){function getConstants(){return _constants}function setConstants(constants){if(!constants)throw new Error("constants must not be empty.");jQuery.extend(!0,_constants,constants)}function setAuthenticatorName(authenticatorName){if(!authenticatorName)throw new Error("authenticatorName must not be empty.");if(!_authenticators[authenticatorName])throw new Error("No authenticator with name '"+authenticatorName+"' registered.");_authenticatorName=authenticatorName}function addAuthenticator(name,authenticator){if(!name)throw new Error("name must not be empty.");if(name===DEFAULT_AUTHENTICATOR_NAME)throw new Error("name must not be equal to '"+DEFAULT_AUTHENTICATOR_NAME+"'.");if(!authenticator)throw new Error("authenticator must not be empty.");if(!angular.isFunction(authenticator.authenticate))throw new Error("authenticator.authenticate must be a function.");_authenticators[name]=authenticator}function getAuthenticator(){return _authenticators[_authenticatorName]}function getAuthenticatorOptions(){return _authenticatorOptions[_authenticatorName]}function setAuthenticatorOptionsFor(name,options){if(!name)throw new Error("name must not be empty.");if(!_authenticators[name])throw new Error("Could not configure unknown authenticator '"+name+"'.");if(!options)throw new Error("config must not be empty.");_authenticatorOptions[name]=options}function login(userName,password){return getAuthenticator().authenticate(userName,password,getAuthenticatorOptions(),_rootScope)}function getCurrentUser(){return _rootScope.user}function isAuthenticated(){return getCurrentUser()?!0:!1}function logout(){return _rootScope.user=null,_rootScope.$broadcast(_constants.EVENT_NAME_LOGOUT),!0}var DEFAULT_AUTHENTICATOR_NAME="defaultAuthenticator",_constants={EVENT_NAME_LOGIN:"sec:login",EVENT_NAME_LOGIN_REQUIRED:"sec:loginRequired",EVENT_NAME_LOGIN_FAILED:"sec:loginFailed",EVENT_NAME_LOGIN_REQUEST:"sec:loginRequest",EVENT_NAME_LOGIN_CONFIRMED:"sec:loginConfirmed",EVENT_NAME_2FA_REQUIRED:"sec:2faRequired",EVENT_NAME_2FA_FAILED:"sec:2faFailed",EVENT_NAME_LOGOUT:"sec:logout"},_httpHeaders=$httpProvider.defaults.headers,_authenticators={};_authenticators[DEFAULT_AUTHENTICATOR_NAME]={authenticate:function(userName,password,options,rootScope){_httpHeaders.common.Authorization="Basic "+_base64Service.encode(userName+":"+password),_http.get(_userServiceEndPoint).success(function(data){rootScope.user=data,rootScope.$broadcast(rootScope.user.authToken?_constants.EVENT_NAME_2FA_REQUIRED:_constants.EVENT_NAME_LOGIN_CONFIRMED)}).error(function(){rootScope.user=null,rootScope.$broadcast(_constants.EVENT_NAME_LOGIN_FAILED)})}};var _authenticatorOptions={};_authenticatorOptions[DEFAULT_AUTHENTICATOR_NAME]={endpoint:"/api/currentUser"};var _base64Service,_rootScope,_authenticatorName=DEFAULT_AUTHENTICATOR_NAME,_http={},_userServiceEndPoint="/api/currentUser";return{$get:function($rootScope,$http,Base64Service){return _rootScope=$rootScope,_base64Service=Base64Service,_http=$http,{getConstants:getConstants,login:login,getCurrentUser:getCurrentUser,isAuthenticated:isAuthenticated,logout:logout}},setConstants:setConstants,setAuthenticatorName:setAuthenticatorName,addAuthenticator:addAuthenticator,setAuthenticatorOptionsFor:setAuthenticatorOptionsFor}}),module.factory("SecurityService",function($rootScope,$http,AuthenticationService){function login(userName,password){return AuthenticationService.isAuthenticated()?void 0:AuthenticationService.login(userName,password)}function logout(){return AuthenticationService.isAuthenticated()?AuthenticationService.logout():!1}function hasRole(role){var user=$rootScope.user;if(!role)return!0;var value=!0;if(0===role.trim().indexOf("!")&&(value=!value,role=role.replace("!","")),user&&user.roles)for(var i=0;i<user.roles.length;i++)if(user.roles[i].key.trim().toLowerCase()===role.trim().toLowerCase())return value;return!value}function hasAnyRole(roles){if(!roles||0===roles.length)return!0;for(var i=0;i<roles.length;i++)if(hasRole(roles[i]))return!0;return!1}function hasAllRoles(roles){if(!roles)return!0;for(var i=0;i<roles.length;i++)if(!hasRole(roles[i]))return!1;return!0}function hasGroup(group){if(!group)return!0;var user=$rootScope.user,value=!0;if(0===group.trim().indexOf("!")&&(value=!value,group=group.replace("!","")),user&&user.groups)for(var i=0;i<user.groups.length;i++)if(user.groups[i].trim().toLowerCase()===group.trim().toLowerCase())return value;return!value}function hasAnyGroup(groups){if(!groups)return!0;for(var i=0;i<groups.length;i++)if(hasGroup(groups[i]))return!0;return!1}function hasAllGroups(groups){if(!groups)return!0;for(var i=0;i<groups.length;i++)if(!hasGroup(groups[i]))return!1;return!0}return{login:login,logout:logout,isAuthenticated:AuthenticationService.isAuthenticated,hasRole:hasRole,hasAnyRole:hasAnyRole,hasAllRoles:hasAllRoles,hasGroup:hasGroup,hasAnyGroup:hasAnyGroup,hasAllGroups:hasAllGroups,getConstants:AuthenticationService.getConstants,getCurrentUser:AuthenticationService.getCurrentUser}});var matcher=new RegExp('"|\\[|]',"g");module.directive("hasRole",["$rootScope","SecurityService",function($rootScope,SecurityService){return{restrict:"A",replace:!1,priority:50,link:function(scope,element,attrs){checkAndHandlePermission(SecurityService,element,SecurityService.hasAnyRole,attrs.hasRole,$rootScope)}}}]),module.directive("auth",["$rootScope","SecurityService",function($rootScope,SecurityService){return{restrict:"A",replace:!1,priority:50,link:function(scope,element){checkAndHandleAuth(SecurityService,$rootScope,element)}}}]),module.directive("notAuth",["$rootScope","SecurityService",function($rootScope,SecurityService){return{restrict:"A",replace:!1,priority:50,link:function(scope,element){checkAndHandleAuth(SecurityService,$rootScope,element,!0)}}}]),module.directive("formLogin",["SecurityService","AuthenticationService",function(SecurityService,AuthenticationService){return{restrict:"EA",replace:!0,templateUrl:"template/security/form-login.html",priority:50,link:function(scope){scope.login=function(){SecurityService.login(scope.loginUsr,scope.loginPwd)},scope.$on(AuthenticationService.getConstants().EVENT_NAME_2FA_REQUIRED,function(){scope.authTokenShow=!0})}}}]),module.directive("formLoginMessage",["$compile","AuthenticationService",function($compile,AuthenticationService){return{restrict:"EA",replace:!0,templateUrl:"template/security/form-login-message.html",priority:50,link:function(scope){scope.formLoginMessage={},scope.formLoginMessage.display=!1,scope.formLoginMessage.severity="info",scope.formLoginMessage.messageTitle="",scope.formLoginMessage.messageContent="",scope.$on(AuthenticationService.getConstants().EVENT_NAME_LOGIN_CONFIRMED,function(){scope.formLoginMessage.display=!0,scope.formLoginMessage.severity="success",scope.formLoginMessage.messageTitle="Login",scope.formLoginMessage.messageContent="Login successful."}),scope.$on(AuthenticationService.getConstants().EVENT_NAME_LOGIN_FAILED,function(){scope.formLoginMessage.display=!0,scope.formLoginMessage.severity="danger",scope.formLoginMessage.messageTitle="Login",scope.formLoginMessage.messageContent="Username and/or password are wrong."}),scope.$on(AuthenticationService.getConstants().EVENT_NAME_2FA_REQUIRED,function(){scope.formLoginMessage.display=!0,scope.formLoginMessage.messageTitle="2FA Authentication",scope.formLoginMessage.messageContent="2FA Verification required."}),scope.$on(AuthenticationService.getConstants().EVENT_NAME_2FA_FAILED,function(){scope.formLoginMessage.display=!0,scope.formLoginMessage.severity="danger",scope.formLoginMessage.messageTitle="2FA Authentication",scope.formLoginMessage.messageContent="2FA Verification failed."})}}}]),module.service("Base64Service",function(){var keyStr="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";this.encode=function(input){for(var chr1,chr2,enc1,enc2,enc3,output="",chr3="",enc4="",i=0;i<input.length;)chr1=input.charCodeAt(i++),chr2=input.charCodeAt(i++),chr3=input.charCodeAt(i++),enc1=chr1>>2,enc2=(3&chr1)<<4|chr2>>4,enc3=(15&chr2)<<2|chr3>>6,enc4=63&chr3,isNaN(chr2)?enc3=enc4=64:isNaN(chr3)&&(enc4=64),output=output+keyStr.charAt(enc1)+keyStr.charAt(enc2)+keyStr.charAt(enc3)+keyStr.charAt(enc4),chr1=chr2=chr3="",enc1=enc2=enc3=enc4="";return output},this.decode=function(input){var chr1,chr2,enc1,enc2,enc3,output="",chr3="",enc4="",i=0;for(input=input.replace(/[^A-Za-z0-9\+\/\=]/g,"");i<input.length;)enc1=keyStr.indexOf(input.charAt(i++)),enc2=keyStr.indexOf(input.charAt(i++)),enc3=keyStr.indexOf(input.charAt(i++)),enc4=keyStr.indexOf(input.charAt(i++)),chr1=enc1<<2|enc2>>4,chr2=(15&enc2)<<4|enc3>>2,chr3=(3&enc3)<<6|enc4,output+=String.fromCharCode(chr1),64!==enc3&&(output+=String.fromCharCode(chr2)),64!==enc4&&(output+=String.fromCharCode(chr3)),chr1=chr2=chr3="",enc1=enc2=enc3=enc4="";return output}})}(),angular.module("alv-ch-ng.security").run(["$templateCache",function($templateCache){"use strict";$templateCache.put("template/security/form-login-message.html",'<div class="alert alert-{{formLoginMessage.severity}} alert-dismissable">\n    <span><strong>{{formLoginMessage.messageTitle}}</strong>&nbsp;{{formLoginMessage.messageContent}}</span>\n    <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>\n</div>'),$templateCache.put("template/security/form-login.html",'<form not-auth>\n    <input form-input form-label="loginUser" type="email" id="loginUser" ng-model="loginUser" />\n    <input form-input form-label="loginPwd" type="password" id="loginPwd" ng-model="loginPwd" />\n    <input form-input form-label="loginAuthToken" form-prepend="Authy" type="number" id="authToken" ng-model="authToken" ng-show="authTokenShow" />\n    <button class="btn-primary" ng-click="login()" ng-show="!authTokenShow">Login</button>\n    <button class="btn-primary" ng-click="authenticate()" glyph-icon="eye-open" ng-show="authTokenShow">Authenticate</button>\n</form>')}]),function(){"use strict";var module=angular.module("alv-ch-ng.i18n",[]);module.provider("I18nPropertyService",function(){function getParamStartMarker(){return _paramStartMarker}function setParamStartMarker(paramStartMarker){if(!paramStartMarker)throw new Error("paramStartMarker must not be empty.");_paramStartMarker=paramStartMarker}function getParamEndMarker(){return _paramEndMarker}function setParamEndMarker(paramEndMarker){if(!paramEndMarker)throw new Error("paramEndMarker must not be empty.");_paramEndMarker=paramEndMarker}function getChangeEventName(){return _changeEventName}function setChangeEventName(changeEventName){if(!changeEventName)throw new Error("changeEventName must not be empty.");_changeEventName=changeEventName}function isMarkUnresolvedProperties(){return _markUnresolvedProperties}function setMarkUnresolvedProperties(markUnresolvedProperties){if(!markUnresolvedProperties&&markUnresolvedProperties!==!1)throw new Error("markUnresolvedProperties must not be empty.");_markUnresolvedProperties=markUnresolvedProperties}function getFilePath(){return _filePath}function setFilePath(filePath){if(null===filePath||void 0===filePath)throw new Error("filePath must not be null or undefined.");_filePath=filePath}function getCurrentLanguage(){return _languageCurrent}function setCurrentLanguage(currentLanguage){if(!currentLanguage)throw new Error("currentLanguage must not be empty.");_languageCurrent=currentLanguage,$.i18n.properties({name:"messages",path:_filePath,mode:"map",language:_languageCurrent,callback:function(){_rootScope.language=_languageCurrent,broadCastChangeToListeners()}})}function getDefaultLanguage(){return _languageDefault}function setDefaultLanguage(defaultLanguage){if(!defaultLanguage)throw new Error("defaultLanguage must not be empty.");_languageDefault=defaultLanguage,_languageCurrent=defaultLanguage,-1===_languagesSupported.indexOf(_languageDefault)&&_languagesSupported.push(_languageDefault)}function getSupportedLanguages(){return _languagesSupported}function setSupportedLanguages(supportedLanguages){if(!supportedLanguages)throw new Error("supportedLanguages must be an array of strings.");if(-1===supportedLanguages.indexOf(_languageDefault))throw new Error("supportedLanguages must contain the default language '"+_languageDefault+"'.");_languagesSupported=supportedLanguages}function broadCastChangeToListeners(){for(var i=0;i<_listeners.length;i++)_listeners[i](_languageCurrent);_rootScope.$broadcast(_changeEventName)}function registerLanguageChangeListener(callback){callback&&_listeners.push(callback)}function removeBrackets(msg){return 0===msg.indexOf("[")&&(msg=msg.slice(1,msg.length)),msg.indexOf("]")===msg.length-1&&(msg=msg.slice(0,-1)),msg}function hasSimpleProperty(key){var property=$.i18n.prop(key);return property==="["+key+"]"?!1:!0}function resolveSimpleProperty(key){var property=$.i18n.prop(key);return isMarkUnresolvedProperties()?property:removeBrackets(property)}function resolveParameterizedProperty(key,map){var property=$.i18n.prop(key);if("string"==typeof map||map instanceof String)try{map=JSON.parse(map)}catch(err){}if(map)for(var attrKey in map){var marker="#{"+attrKey+"}",value=map[attrKey];property=property.replace(marker,value)}return isMarkUnresolvedProperties()?property:removeBrackets(property)}var _changeEventName="i18n:languageChanged",_paramStartMarker="#{",_paramEndMarker="}",_markUnresolvedProperties=!0,_filePath="",_languageDefault="en",_languageCurrent=_languageDefault,_languagesSupported=["en"],_rootScope={},_listeners=[];this.getParamStartMarker=getParamStartMarker,this.setParamStartMarker=setParamStartMarker,this.getParamEndMarker=getParamEndMarker,this.setParamEndMarker=setParamEndMarker,this.getChangeEventName=getChangeEventName,this.setChangeEventName=setChangeEventName,this.isMarkUnresolvedProperties=isMarkUnresolvedProperties,this.setMarkUnresolvedProperties=setMarkUnresolvedProperties,this.getFilePath=getFilePath,this.setFilePath=setFilePath,this.getDefaultLanguage=getDefaultLanguage,this.setDefaultLanguage=setDefaultLanguage,this.getSupportedLanguages=getSupportedLanguages,this.setSupportedLanguages=setSupportedLanguages,this.registerLanguageChangeListener=registerLanguageChangeListener,this.$get=function($rootScope){return _rootScope=$rootScope,setCurrentLanguage(getCurrentLanguage()),{hasSimpleProperty:hasSimpleProperty,resolveSimpleProperty:resolveSimpleProperty,resolveParameterizedProperty:resolveParameterizedProperty,getParamStartMarker:getParamStartMarker,setParamStartMarker:setParamStartMarker,getParamEndMarker:getParamEndMarker,setParamEndMarker:setParamEndMarker,getChangeEventName:getChangeEventName,setChangeEventName:setChangeEventName,isMarkUnresolvedProperties:isMarkUnresolvedProperties,setMarkUnresolvedProperties:setMarkUnresolvedProperties,getFilePath:getFilePath,setFilePath:setFilePath,getDefaultLanguage:getDefaultLanguage,setDefaultLanguage:setDefaultLanguage,getSupportedLanguages:getSupportedLanguages,setSupportedLanguages:setSupportedLanguages,getCurrentLanguage:getCurrentLanguage,setCurrentLanguage:setCurrentLanguage,registerLanguageChangeListener:registerLanguageChangeListener}}}),module.directive("i18nSwitcher",["I18nPropertyService",function(I18nPropertyService){return{restrict:"E",templateUrl:"template/i18n/i18n-switcher.html",replace:!0,link:function(scope,element,attrs){if(scope.styleClass=element.attr("class"),scope.style=element.attr("style"),scope.allLanguages=[],attrs.languages)for(var tokens=attrs.languages.split(","),i=0;i<tokens.length;i++)scope.allLanguages[i]=tokens[i].trim();else scope.allLanguages=I18nPropertyService.getSupportedLanguages();scope.getLanguage=function(){return I18nPropertyService.getCurrentLanguage()},scope.setLanguage=function(language){I18nPropertyService.setCurrentLanguage(language)}}}}]),module.directive("i18nMsg",["I18nPropertyService",function(I18nPropertyService){return{restrict:"A",priority:10,replace:!1,link:function(scope,element,attrs){var key=attrs.i18nMsg;key&&(element.html(I18nPropertyService.resolveSimpleProperty(key)),scope.$root.$on(I18nPropertyService.getChangeEventName(),function(){element.html(I18nPropertyService.resolveSimpleProperty(key))})),scope.$watch(key,function(){key=attrs.i18nMsg,element.html(I18nPropertyService.resolveSimpleProperty(key))})}}}]),module.directive("i18nParamMsg",["I18nPropertyService",function(I18nPropertyService){function updateValues(element,key,props){element.html(props?I18nPropertyService.resolveParameterizedProperty(key,props):I18nPropertyService.resolveSimpleProperty(key))}return{restrict:"A",priority:10,replace:!1,link:function(scope,element,attrs){var key=attrs.i18nParamMsg,props=attrs.i18nProps;key&&updateValues(element,key,props),scope.$root.$on(I18nPropertyService.getChangeEventName(),function(){updateValues(element,key,props)}),scope.$watch("key",function(){key=attrs.i18nParamMsg,updateValues(element,key,props)}),scope.$watch("props",function(){props=attrs.i18nProps,updateValues(element,key,props)})}}}]),module.directive("i18nAttribute",["I18nPropertyService",function(I18nPropertyService){function setAttribute(element,attribute,value){attribute&&element.attr(attribute,I18nPropertyService.resolveSimpleProperty(value))}return{restrict:"A",priority:10,replace:!1,link:function(scope,element,attrs){var i18nValue=attrs.i18nValue,i18nAttribute=attrs.i18nAttribute;scope.$watch("i18nValue",function(){i18nValue=attrs.i18nValue,setAttribute(element,i18nAttribute,i18nValue)}),scope.$watch("i18nAttribute",function(){element.removeAttr(i18nAttribute),i18nAttribute=attrs.i18nAttribute,setAttribute(element,i18nAttribute,i18nValue)}),scope.$root.$on(I18nPropertyService.getChangeEventName(),function(){element.attr(i18nAttribute,I18nPropertyService.resolveSimpleProperty(i18nValue))}),setAttribute(element,i18nAttribute,i18nValue)}}}])}(),angular.module("alv-ch-ng.i18n").run(["$templateCache",function($templateCache){"use strict";$templateCache.put("template/i18n/i18n-switcher.html",'<div id="i18n-switcher">\n    <ul id="i18n-switch">\n        <li id="i18n-switch-option_{{language}}" ng-repeat="language in allLanguages">\n            <a ng-click="setLanguage(language)" ng-class="{\'active\':getLanguage()===language}" i18n-attribute="title" i18n-value="med-i18n.language.short.{{language}}">{{language}}</a>\n        </li>\n    </ul>\n</div>')}]),function(){"use strict";var module=angular.module("alv-ch-ng.ui-core",[]);module.directive("message",function(){return{priority:10,restrict:"E",template:'<div class="alert" ng-transclude></div>',transclude:!0,replace:!0,link:function(scope,element,attrs){var severity="info";void 0!==attrs.messageSeverity&&(severity=attrs.messageSeverity),element.addClass("alert-"+severity),"true"===attrs.messageDismissable&&(element.addClass("alert-dismissable"),element.prepend(attrs.messageDismissableText?angular.element('<button type="button" class="close text-close" data-dismiss="alert" aria-hidden="true">'+attrs.messageDismissableText+"</button>"):angular.element('<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>')))}}}),module.directive("button",function(){return{priority:10,restrict:"E",link:function(scope,element,attrs){element.addClass("btn"),"true"===attrs.primary&&element.addClass("btn-primary")}}}),module.directive("badge",function(){return{priority:10,restrict:"E",template:'<span class="badge" ng-transclude></span>',transclude:!0,replace:!0,link:function(scope,element,attrs){"right"===attrs.badgeAlign&&element.addClass("pull-right")}}}),module.directive("badge",function(){return{priority:10,restrict:"A",replace:!0,link:function(scope,element,attrs){var badge=angular.element('<span class="badge">'+attrs.badge+"</span>");"right"===attrs.badgeAlign&&badge.addClass("pull-right"),element.append(badge)}}}),module.directive("marker",function(){return{priority:10,restrict:"E",template:'<span class="label" ng-transclude></span>',transclude:!0,replace:!0,link:function(scope,element,attrs){element.addClass(attrs.markerSeverity?"label-"+attrs.markerSeverity:"label-default")}}}),module.directive("marker",function(){return{priority:10,restrict:"A",replace:!0,link:function(scope,element,attrs){var marker=angular.element('<span class="label">'+attrs.marker+"</span>");marker.addClass(void 0===attrs.markerSeverity?"label-default":"label-"+attrs.markerSeverity),element.append(marker)}}}),module.directive("glyph",function(){return{priority:10,restrict:"E",template:'<span class="glyphicon" ng-transclude></span>',transclude:!0,replace:!0,link:function(scope,element,attrs){element.addClass(void 0===attrs.icon?"glyphicon-warning-sign":"glyphicon-"+attrs.icon)}}}),module.directive("glyphIcon",function(){return{priority:10,restrict:"A",replace:!0,link:function(scope,element,attrs){var icon=angular.element('<span class="glyphicon"></span>');icon.addClass(attrs.glyphIcon?"glyphicon-"+attrs.glyphIcon:"glyphicon-warning-sign"),"right"===attrs.glyphAlign?element.append(icon):element.prepend(icon)}}}),module.directive("collapsible",function(){return{priority:10,restrict:"A",replace:!0,link:function(scope,element,attrs){element.addClass("pointer"),element.attr("data-toggle","collapse"),element.attr("data-target",attrs.collapsible)}}}),module.directive("grid",["UiRenderService",function(UiRenderService){return{restrict:"EA",priority:110,controller:function($scope){$scope.columns=[],this.addColumn=function(columnObj){$scope.columns.push(columnObj)}},link:function(scope,element,attrs){"show-grid"===attrs.grid&&element.addClass(attrs.grid),scope.$watch("columns",function(columns){var col,_i,_len,_total,row;for(_total=0,row=angular.element('<div class="row"></div>'),_len=columns.length,_i=0;_len>=_i;_i++)_i===_len?element.append(row):(col=columns[_i],_total+=parseInt(col.column),row.append(col.element),(_total>=UiRenderService.getGridDefaultColumns()||col.last===!0)&&(element.append(row),_total=0,row=angular.element('<div class="row"></div>')))})}}}]),module.directive("column",["UiRenderService",function(UiRenderService){return{priority:100,restrict:"A",require:"^grid",replace:!0,link:function(scope,element,attrs,affix){if(attrs.column){var column=+attrs.column,last=!1,gridClass="col-"+UiRenderService.getGridDefaultDevice()+"-";element.addClass(gridClass+column),void 0!==attrs.columnOffset&&(element.addClass(gridClass+"offset-"+attrs.columnOffset),column+=parseInt(attrs.columnOffset)),void 0!==attrs.columnPush&&element.addClass(gridClass+"push-"+attrs.columnPush),void 0!==attrs.columnPull&&element.addClass(gridClass+"pull-"+attrs.columnPull),"true"===attrs.columnLast&&(last=!0),affix.addColumn({column:column,last:last,element:element})}}}}]),module.directive("releaseNote",function(){return{restrict:"A",replace:!0,link:function(scope,element){element.addClass("pointer"),element.click(function(){element.parent().toggleClass("show"),scope.$broadcast("alv-ch-ng:dom-manipulate",{id:element.attr("id"),event:"releaseNote:click",show:element.parent().hasClass("show")})})}}}),module.directive("sidebar",function(){return{restrict:"C",link:function(scope,element){scope.$on("$viewContentLoaded",function(){var height=element.next(".content").outerHeight(!0)||element.prev(".content").outerHeight(!0);element.css("height",height)})}}}),module.directive("sidebarToggle",["$document",function($document){return{restrict:"E",replace:!0,templateUrl:"template/core/sidebar-toggle.html",link:function(scope,element){element.addClass("ng-hide"),element.removeClass("ng-show"),$document.find(".sidebar").length&&(element.addClass("ng-show"),element.removeClass("ng-hide")),scope.$on("$routeChangeSuccess",function(){element.addClass("ng-hide"),element.removeClass("ng-show"),$document.find(".sidebar").length&&(element.addClass("ng-show"),element.removeClass("ng-hide"))})}}}]),module.directive("toc",["$location","$anchorScroll",function($location,$anchorScroll){var collect=function(element,selector){var els=angular.element(selector),tocs=[];return angular.forEach(els,function(el){var local=angular.element(el),toc={};toc.title=local.text(),toc.id=local.attr("id")||local.text().toLowerCase().replace(/[^\w ]+/g,"").replace(/ +/g,"-"),local.attr("id",toc.id),local.addClass("toc-scrollable"),tocs.push(toc)}),tocs};return{priority:20,restrict:"E",replace:!0,templateUrl:"template/core/toc.html",link:function(scope,element,attrs){var selector=attrs.selector||".toc-item";scope.tocs=collect(element,selector),scope.$on("i18n:languageChanged",function(){scope.tocs=collect(element,selector)}),scope.scrollToc=function(anchor){$anchorScroll.yOffset=0;var yOffset=parseInt($("#"+anchor).css("margin-top"))+parseInt($("#"+anchor).css("padding-top")),scrollFixTop=$("body").find("*[scroll-fix]")||!1;angular.forEach(scrollFixTop,function(value){var el=angular.element(value);void 0===el.attr("scroll-fix-bottom")&&el.attr("id")!==element.attr("id")&&el.attr("id")!==element.parent().attr("id")&&(yOffset+=el.outerHeight(!0))}),$anchorScroll.yOffset=yOffset,$anchorScroll(),$location.hash(anchor),scope.$broadcast("alv-ch-ng:dom-manipulate",{id:element.attr("id"),event:"toc:scrollToc"})}}}}]),module.provider("UiConfigService",function(){function setConfig(config){if(!config)throw new Error("Param 'config' must not be empty!");jQuery.extend(!0,_config,config)}function getLabelWidth(){return _config.labelWidth}function getCommonSubmit(){return _config.commonSubmit}function getCommonCancel(){return _config.commonCancel}function getClassPrefix(){return _config.classPrefix}function getTitleSelect(){return _config.titleSelect}function getGridDefaultDevice(){return _config.gridDefaultDevice}function getGridDefaultColumns(){return _config.gridDefaultColumns}function getPageNavigationDefaultColumns(){return _config.pageNavigationDefaultColumns}function getScrollUp(){return _config.scrollUp}function getScrollUpBottom(){return _config.scrollUpBottom}function getScrollUpI18n(){return _config.scrollUpI18n}var _config={labelWidth:2,classPrefix:"alv",gridDefaultDevice:"md",gridDefaultColumns:12,pageNavigationDefaultColumns:2,scrollUp:300,scrollUpBottom:20,scrollUpI18n:"common-i18n.scrollToTop"};return{setConfig:setConfig,$get:function(){return{getLabelWidth:getLabelWidth,getCommonSubmit:getCommonSubmit,getCommonCancel:getCommonCancel,getClassPrefix:getClassPrefix,getTitleSelect:getTitleSelect,getGridDefaultDevice:getGridDefaultDevice,getGridDefaultColumns:getGridDefaultColumns,getPageNavigationDefaultColumns:getPageNavigationDefaultColumns,getScrollUp:getScrollUp,getScrollUpBottom:getScrollUpBottom,getScrollUpI18n:getScrollUpI18n}}}}),module.factory("UiRenderService",["UiConfigService",function(UiConfigService){function getFormGridLabelClass(offset){var spanLabel="col-"+UiConfigService.getGridDefaultDevice()+"-";return"undefined"==typeof offset&&(offset=!1),offset?spanLabel+"offset-"+UiConfigService.getLabelWidth():spanLabel+UiConfigService.getLabelWidth()}function getFormGridContent(){var spanWidth=12-UiConfigService.getLabelWidth();return angular.element('<div class="col-'+UiConfigService.getGridDefaultDevice()+"-"+spanWidth+'"></div>')}function getFormGrid(){var spanWidth=12-UiConfigService.getLabelWidth();return'<div class="col-'+UiConfigService.getGridDefaultDevice()+"-"+spanWidth+'"></div>'}function getCommonSubmit(){return UiConfigService.getCommonSubmit()}function getCommonCancel(){return UiConfigService.getCommonCancel()}function getClassPrefix(cssClass){return UiConfigService.getClassPrefix()+"-"+cssClass}function getTitleSelect(){return UiConfigService.getTitleSelect()}function getGridDefaultDevice(){return UiConfigService.getGridDefaultDevice()}function getGridDefaultColumns(){return UiConfigService.getGridDefaultColumns()}function getPageNavigationDefaultColumns(){return UiConfigService.getPageNavigationDefaultColumns()}function getHelptext(helptext){return angular.element('<span class="help-block" i18n-msg="'+helptext+'"></span>')}function getScrollUp(){return UiConfigService.getScrollUp()}function getScrollUpBottom(){return UiConfigService.getScrollUpBottom()}function getScrollUpI18n(){return UiConfigService.getScrollUpI18n()}return{getFormGridLabelClass:getFormGridLabelClass,getFormGridContent:getFormGridContent,getFormGrid:getFormGrid,getCommonSubmit:getCommonSubmit,getCommonCancel:getCommonCancel,getClassPrefix:getClassPrefix,getTitleSelect:getTitleSelect,getGridDefaultDevice:getGridDefaultDevice,getGridDefaultColumns:getGridDefaultColumns,getHelptext:getHelptext,getPageNavigationDefaultColumns:getPageNavigationDefaultColumns,getScrollUp:getScrollUp,getScrollUpBottom:getScrollUpBottom,getScrollUpI18n:getScrollUpI18n}}])}(),angular.module("alv-ch-ng.ui-core").run(["$templateCache",function($templateCache){"use strict";$templateCache.put("template/core/sidebar-toggle.html",'<button type="button" class="sidebar-toggle" data-toggle="collapse" data-target=".sidebar">\n    <span class="sr-only">Toggle sidebar</span>\n    <span class="icon-bar"></span>\n    <span class="icon-bar"></span>\n    <span class="icon-bar"></span>\n</button>'),$templateCache.put("template/core/toc.html",'<div id="toc">\n    <span i18n-msg="common-i18n.contents"></span>\n    <ul class="toc-list">\n        <li ng-repeat="toc in tocs" spy="{{toc.id}}"><a ng-click="scrollToc(toc.id)">{{toc.title}}</a></li>\n    </ul>\n</div>')}]),function(){"use strict";var module=angular.module("alv-ch-ng.ui-forms",["alv-ch-ng.ui-core","alv-ch-ng.i18n","ui.bootstrap.typeahead","ui.bootstrap.tooltip"]);
module.directive("formPlaceholder",["$compile",function($compile){return{priority:10,restrict:"A",link:function(scope,element,attrs){var placeholderType=attrs.formPlaceholderType;"label"!==placeholderType&&"inside"!==placeholderType&&"outline"!==placeholderType&&(placeholderType="label");var fg=angular.element('<div class="form-group"></div>');fg.addClass("placeholder-"+placeholderType);var placeholder=angular.element('<label class="placeholder-'+placeholderType+'-text" i18n-msg="'+attrs.formPlaceholder+'"></label>');$compile(placeholder)(scope),placeholder.hide(),element.attr("placeholder",placeholder.html()),void 0!==attrs.id&&placeholder.attr("for",attrs.id),element.wrap(fg),element.after(placeholder),scope.$watch(attrs.ngModel,function(newValue){""===newValue||angular.isUndefined(newValue)?(element.next().fadeOut("fast"),element.removeClass("active")):(element.addClass("active"),element.next().fadeIn("fast"))})}}}]),module.directive("formSelect",["I18nPropertyService",function(I18nPropertyService){var setProperties=function(){return I18nPropertyService.hasSimpleProperty("common-i18n.select.selectedTextFormat")&&I18nPropertyService.hasSimpleProperty("common-i18n.select.noneSelectedText")&&I18nPropertyService.hasSimpleProperty("common-i18n.select.noneResultsText")&&I18nPropertyService.hasSimpleProperty("common-i18n.select.countSelectedText")&&I18nPropertyService.hasSimpleProperty("common-i18n.select.maxOptionsText1")&&I18nPropertyService.hasSimpleProperty("common-i18n.select.maxOptionsText2")&&I18nPropertyService.hasSimpleProperty("common-i18n.select.maxOptionsText3")&&I18nPropertyService.hasSimpleProperty("common-i18n.select.maxOptionsText4")?{style:"btn-default",size:"auto",title:null,selectedTextFormat:I18nPropertyService.resolveSimpleProperty("common-i18n.select.selectedTextFormat"),noneSelectedText:I18nPropertyService.resolveSimpleProperty("common-i18n.select.noneSelectedText"),noneResultsText:I18nPropertyService.resolveSimpleProperty("common-i18n.select.noneResultsText"),countSelectedText:I18nPropertyService.resolveSimpleProperty("common-i18n.select.countSelectedText"),maxOptionsText:[I18nPropertyService.resolveSimpleProperty("common-i18n.select.maxOptionsText1"),I18nPropertyService.resolveSimpleProperty("common-i18n.select.maxOptionsText2"),[I18nPropertyService.resolveSimpleProperty("common-i18n.select.maxOptionsText3"),I18nPropertyService.resolveSimpleProperty("common-i18n.select.maxOptionsText4")]],width:!1,container:!1,hideDisabled:!1,showSubtext:!1,showIcon:!1,showContent:!0,dropupAuto:!1,header:!1,liveSearch:!1,multipleSeparator:", ",iconBase:"glyphicon",tickIcon:"glyphicon-ok"}:void 0};return{priority:10,restrict:"A",link:function(scope,element,attrs){element.addClass("form-control"),scope.$watchCollection(attrs.selectpicker,function(){element.selectpicker(setProperties()),element.selectpicker("render")}),scope.$watch(attrs.ngModel,function(){element.selectpicker("refresh")}),scope.$root.$on("i18n:languageChanged",function(){element.selectpicker(setProperties()),element.selectpicker("render")})}}}]),module.directive("formTextarea",function(){return{priority:20,restrict:"A",link:function(scope,element,attrs){element.addClass("form-control"),angular.isString(attrs.formTextareaResize)&&(element.addClass("form-resize-"+attrs.formTextareaResize),element.parent().hasClass("form-group")&&void 0!==attrs.formTextareaOverflow&&(element.addClass("form-overflow"),element.parent().css("height",element.parent().outerHeight(!0))))}}}),module.directive("formInput",function(){return{restrict:"A",priority:10,link:function(scope,element){element.addClass("form-control")}}}),module.directive("formLabel",["$compile","UiRenderService",function($compile,UiRenderService){return{restrict:"A",priority:10,link:function(scope,element,attrs){var inlineForm=element.parent().hasClass("form-inline"),horizontalForm=element.parent().hasClass("form-horizontal"),fg=angular.element('<div class="form-group"></div>'),cb=angular.element("<div></div>"),horizontalFormLayer=UiRenderService.getFormGridContent();("checkbox"===attrs.type||"radio"===attrs.type)&&(cb.addClass(attrs.type),cb.addClass(UiRenderService.getClassPrefix("check")));var labelText=angular.element('<span i18n-msg="'+attrs.formLabel+'"></span>');$compile(labelText)(scope);var helpText;void 0!==attrs.formHelptext&&(helpText=UiRenderService.getHelptext(attrs.formHelptext),$compile(helpText)(scope)),(horizontalForm||"checkbox"!==attrs.type&&"radio"!==attrs.type)&&element.wrap(fg),("checkbox"===attrs.type||"radio"===attrs.type)&&(horizontalForm&&(horizontalFormLayer.addClass(UiRenderService.getFormGridLabelClass(!0)),element.wrap(horizontalFormLayer)),element.wrap(cb));var label=angular.element('<label for="'+attrs.id+'"></label>');horizontalForm&&"checkbox"!==attrs.type&&"radio"!==attrs.type&&label.addClass(UiRenderService.getFormGridLabelClass()).addClass("control-label"),inlineForm&&"checkbox"!==attrs.type&&"radio"!==attrs.type&&label.addClass("sr-only"),"checkbox"!==attrs.type&&"radio"!==attrs.type?(label.append(labelText),element.before(label),horizontalForm&&element.wrap(horizontalFormLayer)):(labelText.addClass(attrs.type+"-label"),label.append(labelText),label.append(angular.element('<span class="box"></span>')),element.after(label)),void 0===attrs.formHelptext||horizontalForm||inlineForm?void 0!==attrs.formHelptext&&horizontalForm?element.after(helpText):void 0!==attrs.formHelptext&&inlineForm&&(helpText.addClass("sr-only"),element.after(helpText)):element.before(helpText),void 0!==attrs.ngShow&&scope.$watch(attrs.ngShow,function(display){var _element=element.parent();(horizontalForm||attrs.formPrepend||attrs.formAppend||attrs.formPrependDropdown||attrs.formAppendDropdown)&&(_element=_element.parent()),display===!0||"true"===display?(_element.addClass("ng-show"),_element.removeClass("ng-hide")):(_element.addClass("ng-hide"),_element.removeClass("ng-show"))}),void 0!==attrs.ngHide&&scope.$watch(attrs.ngHide,function(hide){var _element=element.parent();(horizontalForm||attrs.formPrepend||attrs.formAppend||attrs.formPrependDropdown||attrs.formAppendDropdown)&&(_element=_element.parent()),hide===!0||"true"===hide?(_element.addClass("ng-hide"),_element.removeClass("ng-show")):(_element.addClass("ng-show"),_element.removeClass("ng-hide"))})}}}]),module.directive("formTag",["$compile","$parse",function($compile,$parse){return{restrict:"A",priority:20,link:function(scope,element,attrs){var targetModel=$parse(attrs.formTargetModel)(scope),fg=angular.element('<div class="form-control"></div>'),tagLayer=angular.element("<span></span>"),tagLayerItem=angular.element('<div class="tag" ng-repeat="item in '+attrs.formTargetModel+'">{{item.text}}<button type="button" data-dismiss="alert" aria-hidden="true" class="close" ng-click="removeFromModel($index)">&times;</button></div>');$compile(tagLayerItem)(scope),tagLayer.append(tagLayerItem),element.wrap(fg),element.before(tagLayer),element.addClass("tag-input"),scope.removeFromModel=function(tagIndex){targetModel.splice(tagIndex,1)},scope.addToTargetModel=function(newObject){var duplicate=!1;angular.forEach(targetModel,function(value){angular.equals(newObject,value)&&(duplicate=!0)}),duplicate||targetModel.push(newObject)}}}}]),module.directive("formAppendDropdown",["$compile","$parse",function($compile,$parse){return{restrict:"A",priority:10,link:function(scope,element,attrs){var targetModel=$parse(attrs.formTargetModel)(scope),inputGroup=angular.element('<div class="input-group"></div>'),inputGroupAppend=angular.element('<span class="input-group-btn"></span>'),inputGroupDropdown=angular.element('<div class="btn-group"></div>'),dropdownTitle="";void 0!==attrs.formAppendDropdownTitle&&(dropdownTitle=attrs.formAppendDropdownTitle+" ");var dropdownButton=angular.element('<button class="dropdown-toggle btn" type="button" data-toggle="dropdown">'+dropdownTitle+'<span class="caret"></span></button>'),dropdownList=angular.element('<ul class="dropdown-menu dropdown-menu-right"></ul>'),dropdownListItem=angular.element('<li ng-repeat="item in '+attrs.formAppendDropdown+'"><a ng-click="addToTargetModel(item)" class="pointer">{{item.text}}</a></li>');$compile(dropdownListItem)(scope),dropdownList.append(dropdownListItem),inputGroupDropdown.append(dropdownButton),inputGroupDropdown.append(dropdownList),inputGroupAppend.append(inputGroupDropdown),element.parent().hasClass("input-group")||element.wrap(inputGroup),element.after(inputGroupAppend),scope.addToTargetModel=function(newObject){var duplicate=!1;angular.forEach(targetModel,function(value){angular.equals(newObject,value)&&(duplicate=!0)}),duplicate||targetModel.push(newObject)}}}}]),module.directive("formPrependDropdown",["$compile","$parse",function($compile,$parse){return{restrict:"A",priority:10,link:function(scope,element,attrs){var targetModel=$parse(attrs.formTargetModel)(scope),inputGroup=angular.element('<div class="input-group"></div>'),inputGroupPrepend=angular.element('<span class="input-group-btn"></span>'),inputGroupDropdown=angular.element('<div class="btn-group"></div>'),dropdownTitle="";void 0!==attrs.formPrependDropdownTitle&&(dropdownTitle=attrs.formPrependDropdownTitle+" ");var dropdownButton=angular.element('<button class="dropdown-toggle btn" type="button" data-toggle="dropdown">'+dropdownTitle+'<span class="caret"></span></button>'),dropdownList=angular.element('<ul class="dropdown-menu dropdown-menu-right"></ul>'),dropdownListItem=angular.element('<li ng-repeat="item in '+attrs.formPrependDropdown+'"><a ng-click="addToTargetModel(item)" class="pointer">{{item.text}}</a></li>');$compile(dropdownListItem)(scope),dropdownList.append(dropdownListItem),inputGroupDropdown.append(dropdownButton),inputGroupDropdown.append(dropdownList),inputGroupPrepend.append(inputGroupDropdown),element.parent().hasClass("input-group")||element.wrap(inputGroup),element.before(inputGroupPrepend),scope.addToTargetModel=function(newObject){var duplicate=!1;angular.forEach(targetModel,function(value){angular.equals(newObject,value)&&(duplicate=!0)}),duplicate||targetModel.push(newObject)}}}}]),module.directive("formAppend",["$compile",function($compile){return{restrict:"A",priority:20,link:function(scope,element,attrs){var inputGroup=angular.element('<div class="input-group"></div>'),inputGroupAppend=angular.element('<span class="input-group-addon"></span>');if(void 0===attrs.formAppendIcon&&void 0===attrs.formAppendBtn){var appendText=angular.element('<span i18n-msg="'+attrs.formAppend+'"></span>');$compile(appendText)(scope),inputGroupAppend.append(appendText)}else"true"===attrs.formAppendIcon&&void 0===attrs.formAppendBtn&&(inputGroupAppend.attr("glyph-icon",attrs.formAppend),$compile(inputGroupAppend)(scope));if(void 0!==attrs.formAppendBtn){var appendBtnType="button";void 0!==attrs.formAppendBtnSubmit&&"true"===attrs.formAppendBtnSubmit&&(appendBtnType="submit");var appendBtn=angular.element('<button type="'+appendBtnType+'" ng-click="'+attrs.formAppendBtn+'"></button>');"true"===attrs.formAppendIcon?appendBtn.attr("glyph-icon",attrs.formAppend):appendBtn.attr("i18n-msg",attrs.formAppend),$compile(appendBtn)(scope),inputGroupAppend=angular.element('<span class="input-group-btn"></span>'),inputGroupAppend.append(appendBtn)}element.parent().hasClass("input-group")||element.wrap(inputGroup),element.after(inputGroupAppend)}}}]),module.directive("formPrepend",["$compile",function($compile){return{restrict:"A",priority:20,link:function(scope,element,attrs){var inputGroup=angular.element('<div class="input-group"></div>'),inputGroupPrepend=angular.element('<span class="input-group-addon"></span>');if(void 0===attrs.formPrependIcon&&void 0===attrs.formPrependBtn){var prependText=angular.element('<span i18n-msg="'+attrs.formPrepend+'"></span>');$compile(prependText)(scope),inputGroupPrepend.append(prependText)}else"true"===attrs.formPrependIcon&&void 0===attrs.formPrependBtn&&(inputGroupPrepend.attr("glyph-icon",attrs.formPrepend),$compile(inputGroupPrepend)(scope));if(void 0!==attrs.formPrependBtn){var prependBtnType="button";void 0!==attrs.formPrependBtnSubmit&&"true"===attrs.formPrependBtnSubmit&&(prependBtnType="submit");var prependBtn=angular.element('<button type="'+prependBtnType+'" ng-click="'+attrs.formPrependBtn+'"></button>');"true"===attrs.formPrependIcon?prependBtn.attr("glyph-icon",attrs.formPrepend):prependBtn.attr("i18n-msg",attrs.formPrepend),$compile(prependBtn)(scope),inputGroupPrepend=angular.element('<span class="input-group-btn"></span>'),inputGroupPrepend.append(prependBtn)}element.parent().hasClass("input-group")||element.wrap(inputGroup),element.before(inputGroupPrepend)}}}]),module.directive("formInset",["$compile",function($compile){return{restrict:"A",priority:20,link:function(scope,element,attrs){var inputGroup=angular.element('<div class="input-group"></div>'),inputGroupPrepend=angular.element('<span class="input-group-addon input-group-inset"></span>');if(void 0===attrs.formInsetAction&&void 0===attrs.formInsetBtn)inputGroupPrepend.attr("glyph-icon",attrs.formInset),$compile(inputGroupPrepend)(scope);else{var insetAction=angular.element('<a ng-click="'+attrs.formInsetAction+'"></a>');if(void 0!==attrs.formInsetBtn){var insetBtnType="button";"submit"===attrs.formInsetBtn&&(insetBtnType="submit"),insetAction=angular.element('<button type="'+insetBtnType+'" ng-click="'+attrs.formInsetAction+'"></button>')}insetAction.attr("glyph-icon",attrs.formInset),$compile(insetAction)(scope),inputGroupPrepend.append(insetAction)}element.parent().hasClass("input-group")||element.wrap(inputGroup),element.before(inputGroupPrepend)}}}])}(),angular.module("alv-ch-ng.ui-forms").run(["$templateCache",function($templateCache){"use strict";$templateCache.put("template/typeahead/typeahead-match.html",'<a tabindex="-1" bind-html-unsafe="match.label | typeaheadHighlight:query"></a>'),$templateCache.put("template/typeahead/typeahead-popup.html",'<ul class="dropdown-menu" ng-if="isOpen()" ng-style="{top: position.top+\'px\', left: position.left+\'px\'}" style="display: block;" role="listbox" aria-hidden="{{!isOpen()}}">\n    <li ng-repeat="match in matches track by $index" ng-class="{active: isActive($index) }" ng-mouseenter="selectActive($index)" ng-click="selectMatch($index)" role="option" id="{{match.id}}">\n        <div typeahead-match index="$index" match="match" query="query" template-url="templateUrl"></div>\n    </li>\n</ul>'),$templateCache.put("template/tooltip/tooltip-html-unsafe-popup.html",'<div class="tooltip {{placement}}" ng-class="{ in: isOpen(), fade: animation() }">\n  <div class="tooltip-arrow"></div>\n  <div class="tooltip-inner" bind-html-unsafe="content"></div>\n</div>\n'),$templateCache.put("template/tooltip/tooltip-popup.html",'<div class="tooltip {{placement}}" ng-class="{ in: isOpen(), fade: animation() }">\n  <div class="tooltip-arrow"></div>\n  <div class="tooltip-inner" ng-bind="content"></div>\n</div>\n')}]),function(){"use strict";var module=angular.module("alv-ch-ng.ui-scroll",["alv-ch-ng.ui-core","alv-ch-ng.i18n"]);module.service("ScrollService",["$window",function($window){var ctrl=this;ctrl.scrollY=function(){return $window.scrollY?$window.scrollY:$window.pageYOffset?$window.pageYOffset:$window.document.documentElement&&document.documentElement.scrollTop?document.documentElement.scrollTop:$window.document.body.scrollTop?document.body.scrollTop:0}}]),module.factory("ScrollSpyService",function(){function addSpy(spyObj){spies[spyObj.id]=spyObj}function getSpies(){return spies}function clearSpies(){spies={}}var spies={};return{addSpy:addSpy,getSpies:getSpies,clearSpies:clearSpies}}),module.controller("ScrollSpyCtrl",["$scope","ScrollService","ScrollSpyService",function($scope,ScrollService,ScrollSpyService){var ctrl=this;ctrl.checkSpy=function(bodyOffset){var highlightSpy,pos,spyOffset;highlightSpy=null,angular.forEach(ScrollSpyService.getSpies(),function(spy){spy.out(),spyOffset=parseInt($("#"+spy.id).css("margin-top"))+parseInt($("#"+spy.id).css("padding-top")),pos=$("#"+spy.id).offset().top-bodyOffset-spyOffset,pos-ScrollService.scrollY()<=0&&(spy.pos=pos,null==highlightSpy&&(highlightSpy=spy),highlightSpy.pos<spy.pos&&(highlightSpy=spy))}),null!==highlightSpy&&highlightSpy["in"]()}}]),module.controller("ScrollFixCtrl",["$scope","$window","$document","ScrollService",function($scope,$window,$document,ScrollService){var ctrl=this,init=!0,fixed=!1,fixedTopBottom=!1,parentFixed=!1,param={};ctrl.parentSet=function(el,id,attrScrollFix){id===attrScrollFix&&ctrl.doParentSet(el,attrScrollFix)},ctrl.doParentSet=function(el,attrScrollFix){ctrl.checkParent(attrScrollFix),param.overlayTop=ctrl.overlayTop(attrScrollFix),ctrl.checkElementTop(el),ctrl.refreshDummyElement(el)},ctrl.checkParent=function(attrScrollFix){parentFixed=!1,$("#"+attrScrollFix).hasClass("scroll-fixed")&&(parentFixed=!0)},ctrl.getDummyElement=function(el){var element=angular.element('<div id="scroll-fix-'+param.elementId+'" style="height: '+el.outerHeight(!0)+'px;"></div>');return"100%"!==param.elementWidth&&element.css({width:param.elementWidth,"float":param.elementFloat}),element},ctrl.refreshDummyElement=function(el){$("#scroll-fix-"+param.elementId).css("height",el.outerHeight(!0))},ctrl.elementTop=function(el){return el.offset().top-parseInt(el.css("margin-top"))-parseInt(el.css("padding-top"))},ctrl.overlayTop=function(attrScrollFix){return attrScrollFix&&"undefined"!==$("#"+attrScrollFix).offset()?$("#"+attrScrollFix).outerHeight(!0)+$("#"+attrScrollFix).offset().top-ScrollService.scrollY():0},ctrl.overlayBottom=function(el,attrScrollFixBottom){return attrScrollFixBottom?$document.height()-$("#"+attrScrollFixBottom).outerHeight(!0):0},ctrl.elementTopPosition=function(attrScrollFix){return attrScrollFix?$("#"+attrScrollFix).outerHeight(!0):0},ctrl.fixElement=function(el){el.addClass("scroll-fixed"),el.css("width",param.elementWidth),angular.isString(param.attrScrollFixBottom)&&!param.attrScrollFix?(el.css("bottom",0),el.before(ctrl.getDummyElement(el))):(el.css("top",ctrl.overlayTop(param.attrScrollFix)),el.after(ctrl.getDummyElement(el))),fixed=!0,$scope.$broadcast("alv-ch-ng:dom-manipulate",{id:param.elementId,event:"scrollFix:fixed"})},ctrl.releaseElement=function(el){el.removeClass("scroll-fixed"),$("#scroll-fix-"+param.elementId).remove(),angular.isString(param.attrScrollFixBottom)&&!param.attrScrollFix?el.css("bottom","auto"):el.css("top","auto"),fixed=!1,$scope.$broadcast("alv-ch-ng:dom-manipulate",{id:param.elementId,event:"scrollFix:release"})},ctrl.fixTopBottomElement=function(el){el.removeAttr("style"),el.removeClass("scroll-fixed"),el.addClass("scroll-fixed-bottom"),el.css("bottom",parseInt(el.css("margin-bottom"))+parseInt(el.css("padding-bottom"))),fixedTopBottom=!0,$scope.$broadcast("alv-ch-ng:dom-manipulate",{id:param.elementId,event:"scrollFix:topBottom:fixed"})},ctrl.releaseTopBottomElement=function(el){el.removeAttr("style"),el.removeClass("scroll-fixed-bottom"),el.addClass("scroll-fixed"),el.css("top",ctrl.overlayTop(param.attrScrollFix)),fixedTopBottom=!1,$scope.$broadcast("alv-ch-ng:dom-manipulate",{id:param.elementId,event:"scrollFix:topBottom:released"})},ctrl.resizeElement=function(el,resizedWidth){param.elementWidth=resizedWidth,el.css("width",resizedWidth),$scope.$broadcast("alv-ch-ng:dom-manipulate",{id:param.elementId,event:"scrollFix:resize"})},ctrl.checkElement=function(el){ctrl.checkElementTop(el),ctrl.checkFix(el)&&ctrl.fixElement(el),ctrl.checkRelease(el)&&ctrl.releaseElement(el),ctrl.checkTopBottom(el)},ctrl.checkElementTop=function(el){param.elementTop===ctrl.elementTop(el)||fixed||(param.elementTop=ctrl.elementTop(el))},ctrl.checkElementWidth=function(el){var resizedWidth=$("#scroll-fix-"+param.elementId).outerWidth(!0)||"100%";"100%"!==param.elementWidth&&resizedWidth!==param.elementWidth&&ctrl.resizeElement(el,resizedWidth),ctrl.doParentSet(el,param.attrScrollFix)},ctrl.checkFix=function(el){if(!fixed){if(ScrollService.scrollY()+ctrl.overlayTop(param.attrScrollFix)>ctrl.elementTop(el)&&(ctrl.checkParent(param.attrScrollFix),angular.isString(param.attrScrollFix)&&parentFixed||!param.attrScrollFix))return!0;if(angular.isString(param.attrScrollFixBottom)&&!param.attrScrollFix&&ScrollService.scrollY()+$(window).height()<=ctrl.overlayBottom(el,param.attrScrollFixBottom))return!0}return!1},ctrl.checkRelease=function(el){if(fixed){if(angular.isString(param.attrScrollFix)&&param.elementTop>ScrollService.scrollY()+param.overlayTop)return!0;if(angular.isString(param.attrScrollFixBottom)&&!param.attrScrollFix&&ScrollService.scrollY()+$(window).height()>ctrl.overlayBottom(el,param.attrScrollFixBottom))return!0}return!1},ctrl.checkTopBottom=function(el){if(angular.isString(param.attrScrollFix)&&angular.isString(param.attrScrollFixBottom)){var overlayTop=$document.height()-($("#"+param.attrScrollFixBottom).outerHeight(!0)+el.outerHeight(!0)+parseInt(el.css("margin-bottom"))+parseInt(el.css("padding-bottom"))),overlayBottom=$document.height()-($("#"+param.attrScrollFixBottom).outerHeight(!0)+el.outerHeight(!0)+parseInt(el.css("margin-bottom"))+parseInt(el.css("padding-bottom"))+ctrl.overlayTop(param.attrScrollFix));overlayBottom<=ScrollService.scrollY()&&!fixedTopBottom&&ctrl.fixTopBottomElement(el),overlayTop>ScrollService.scrollY()+ctrl.overlayTop(param.attrScrollFix)&&fixedTopBottom&&ctrl.releaseTopBottomElement(el)}},ctrl.init=function(el,_param){param=_param,param.overlayTop=ctrl.overlayTop(param.attrScrollFix),param.attrScrollFixBottom||ctrl.fixElement(el),ctrl.checkElement(el),init=!1}}]),module.directive("scrollFix",["$window","$document",function($window,$document){return{priority:10,restrict:"A",controller:"ScrollFixCtrl",link:function(scope,element,attrs,ctrl){var attrScrollFix=attrs.scrollFix||!1,attrScrollFixBottom=attrs.scrollFixBottom||!1,elementId=attrs.id||"random_"+Math.floor(6*Math.random()+1);void 0===attrs.id&&element.attr("id",elementId);var elementTop=ctrl.elementTop(element),elementHeight=element.outerHeight(!0),elementWidth="100%";element.outerWidth(!0)<$document.width()&&(elementWidth=element.outerWidth(!0));var elementFloat=element.css("float"),param={attrScrollFix:attrScrollFix,attrScrollFixBottom:attrScrollFixBottom,elementId:elementId,elementTop:elementTop,elementHeight:elementHeight,elementWidth:elementWidth,elementFloat:elementFloat,overlayTop:"auto"};$($window).scroll(function(){ctrl.checkElement(element)}),$($window).resize(function(){ctrl.checkElementWidth(element)}),$document.ready(function(){ctrl.init(element,param)}),scope.$on("$viewContentLoaded",function(){ctrl.checkElement(element)}),scope.$on("alv-ch-ng:dom-manipulate",function(event,trigger){ctrl.parentSet(element,trigger.id,attrs.scrollFix)})}}}]),module.directive("scrollSpy",["$window","ScrollSpyService",function($window,ScrollSpyService){return{restrict:"A",priority:20,controller:"ScrollSpyCtrl",link:function(scope,elem,attrs,ctrl){var bodyOffset=0;angular.forEach($("body").find("*[scroll-fix]"),function(value){var el=angular.element(value);void 0===el.attr("scroll-fix-bottom")&&(bodyOffset+=el.outerHeight(!0))}),$($window).scroll(function(){ctrl.checkSpy(bodyOffset)}),scope.$on("i18n:languageChanged",function(){ScrollSpyService.clearSpies()}),scope.$on("$routeChangeSuccess",function(){ScrollSpyService.clearSpies()})}}}]),module.directive("spy",["ScrollSpyService",function(ScrollSpyService){return{restrict:"A",priority:30,link:function(scope,elem,attrs){ScrollSpyService.addSpy({id:attrs.spy,"in":function(){elem.addClass("active")},out:function(){elem.removeClass("active")}})}}}]),module.directive("scrollUp",["$compile","$window","$document","$location","$anchorScroll","ScrollService","UiRenderService",function($compile,$window,$document,$location,$anchorScroll,ScrollService,UiRenderService){return{restrict:"A",priority:10,link:function(scope,element,attrs){var body=element;"BODY"!==element.prop("tagName")&&(body=$("body"));var documentTopId=element.attr("id")||"document-top";void 0===body.attr("id")&&body.attr("id",documentTopId);var fixed=!1,scrollFix=attrs.scrollUp||!1,scrollFixBottom=$("*[scroll-fix-bottom]").attr("id")||!1,scrollUp=angular.element('<a id="scroll-up" ng-click="scrollTop()" i18n-msg="'+UiRenderService.getScrollUpI18n()+'" style="display: none; bottom: '+UiRenderService.getScrollUpBottom()+'px;"></a>');$compile(scrollUp)(scope),body.append(scrollUp);var fixedBottom=$("#"+scrollFix).outerHeight(!0)+$("#scroll-up").outerHeight(!0)+UiRenderService.getScrollUpBottom();angular.isString(scrollFixBottom)&&(fixedBottom-=$("#"+scrollFixBottom).outerHeight(!0)),$($window).scroll(function(){var overlay=$document.height()-$("#"+scrollFix).outerHeight(!0);angular.isString(scrollFixBottom)&&(overlay+=$("#"+scrollFixBottom).outerHeight(!0)),ScrollService.scrollY()>UiRenderService.getScrollUp()&&!fixed?($("#scroll-up").css("display","block"),fixed=!0):ScrollService.scrollY()<UiRenderService.getScrollUp()&&fixed&&($("#scroll-up").css("display","none"),fixed=!1),ScrollService.scrollY()+$(window).height()>overlay?($("#scroll-up").css("bottom",fixedBottom),$("#scroll-up").css("position","relative")):($("#scroll-up").css("bottom",UiRenderService.getScrollUpBottom()),$("#scroll-up").css("position","fixed"))}),scope.scrollTop=function(){$anchorScroll(),$location.hash(documentTopId),scope.$broadcast("alv-ch-ng:dom-manipulate",{id:documentTopId,event:"scrollUp:scrollTop"})}}}}])}(),function(window,document){function n(a){return function(){return this[a]}}function r(a,b){var c=a.split("."),d=q;!(c[0]in d)&&d.execScript&&d.execScript("var "+c[0]);for(var e;c.length&&(e=c.shift());)c.length||void 0===b?d=d[e]?d[e]:d[e]={}:d[e]=b}function aa(a){return a.call.apply(a.bind,arguments)}function ba(a,b){if(!a)throw Error();if(2<arguments.length){var d=Array.prototype.slice.call(arguments,2);return function(){var c=Array.prototype.slice.call(arguments);return Array.prototype.unshift.apply(c,d),a.apply(b,c)}}return function(){return a.apply(b,arguments)}}function t(){return t=Function.prototype.bind&&-1!=Function.prototype.bind.toString().indexOf("native code")?aa:ba,t.apply(l,arguments)}function v(a,b){this.G=a,this.v=b||a,this.z=this.v.document}function ca(a,b,c){a=a.z.getElementsByTagName(b)[0],a||(a=document.documentElement),a&&a.lastChild&&a.insertBefore(c,a.lastChild)}function w(a,b){for(var c=a.className.split(/\s+/),d=0,e=c.length;e>d;d++)if(c[d]==b)return;c.push(b),a.className=c.join(" ").replace(/\s+/g," ").replace(/^\s+|\s+$/,"")}function x(a,b){for(var c=a.className.split(/\s+/),d=[],e=0,g=c.length;g>e;e++)c[e]!=b&&d.push(c[e]);a.className=d.join(" ").replace(/\s+/g," ").replace(/^\s+|\s+$/,"")}function da(a,b){for(var c=a.className.split(/\s+/),d=0,e=c.length;e>d;d++)if(c[d]==b)return j;return m}function y(a){var b=a.v.location.protocol;return"about:"==b&&(b=a.G.location.protocol),"https:"==b?"https:":"http:"}function ea(a,b){var c=a.createElement("link",{rel:"stylesheet",href:b}),d=m;c.onload=function(){d||(d=j)},c.onerror=function(){d||(d=j)},ca(a,"head",c)}function z(a,b,c,d){var e=a.z.getElementsByTagName("head")[0];if(e){var g=a.createElement("script",{src:b}),f=m;return g.onload=g.onreadystatechange=function(){f||this.readyState&&"loaded"!=this.readyState&&"complete"!=this.readyState||(f=j,c&&c(l),g.onload=g.onreadystatechange=l,"HEAD"==g.parentNode.tagName&&e.removeChild(g))},e.appendChild(g),window.setTimeout(function(){f||(f=j,c&&c(Error("Script load timeout")))},d||5e3),g}return l}function A(a,b,c){this.M=a,this.U=b,this.Aa=c}function B(a,b,c,d){this.d=a!=l?a:l,this.o=b!=l?b:l,this.aa=c!=l?c:l,this.f=d!=l?d:l}function C(a){a=fa.exec(a);var b=l,c=l,d=l,e=l;return a&&(a[1]!==l&&a[1]&&(b=parseInt(a[1],10)),a[2]!==l&&a[2]&&(c=parseInt(a[2],10)),a[3]!==l&&a[3]&&(d=parseInt(a[3],10)),a[4]!==l&&a[4]&&(e=/^[0-9]+$/.test(a[4])?parseInt(a[4],10):a[4])),new B(b,c,d,e)}function D(a,b,c,d,e,g,f,h,k,p,s){this.K=a,this.Ga=b,this.za=c,this.fa=d,this.Ea=e,this.ea=g,this.wa=f,this.Fa=h,this.va=k,this.da=p,this.k=s}function E(a,b){this.a=a,this.I=b}function F(a){var b=H(a.a,/(iPod|iPad|iPhone|Android|Windows Phone|BB\d{2}|BlackBerry)/,1);return""!=b?(/BB\d{2}/.test(b)&&(b="BlackBerry"),b):(a=H(a.a,/(Linux|Mac_PowerPC|Macintosh|Windows|CrOS)/,1),""!=a?("Mac_PowerPC"==a&&(a="Macintosh"),a):"Unknown")}function G(a){var b=H(a.a,/(OS X|Windows NT|Android) ([^;)]+)/,2);if(b||(b=H(a.a,/Windows Phone( OS)? ([^;)]+)/,2))||(b=H(a.a,/(iPhone )?OS ([\d_]+)/,2)))return b;if(b=H(a.a,/(?:Linux|CrOS) ([^;)]+)/,1))for(var b=b.split(/\s/),c=0;c<b.length;c+=1)if(/^[\d\._]+$/.test(b[c]))return b[c];return(a=H(a.a,/(BB\d{2}|BlackBerry).*?Version\/([^\s]*)/,2))?a:"Unknown"}function ha(a){var b=F(a),c=G(a),d=C(c),e=H(a.a,/AppleWeb(?:K|k)it\/([\d\.\+]+)/,1),g=C(e),f="Unknown",h=new B,k="Unknown",p=m;return/OPR\/[\d.]+/.test(a.a)?f="Opera":-1!=a.a.indexOf("Chrome")||-1!=a.a.indexOf("CrMo")||-1!=a.a.indexOf("CriOS")?f="Chrome":/Silk\/\d/.test(a.a)?f="Silk":"BlackBerry"==b||"Android"==b?f="BuiltinBrowser":-1!=a.a.indexOf("PhantomJS")?f="PhantomJS":-1!=a.a.indexOf("Safari")?f="Safari":-1!=a.a.indexOf("AdobeAIR")&&(f="AdobeAIR"),"BuiltinBrowser"==f?k="Unknown":"Silk"==f?k=H(a.a,/Silk\/([\d\._]+)/,1):"Chrome"==f?k=H(a.a,/(Chrome|CrMo|CriOS)\/([\d\.]+)/,2):-1!=a.a.indexOf("Version/")?k=H(a.a,/Version\/([\d\.\w]+)/,1):"AdobeAIR"==f?k=H(a.a,/AdobeAIR\/([\d\.]+)/,1):"Opera"==f?k=H(a.a,/OPR\/([\d.]+)/,1):"PhantomJS"==f&&(k=H(a.a,/PhantomJS\/([\d.]+)/,1)),h=C(k),p="AdobeAIR"==f?2<h.d||2==h.d&&5<=h.o:"BlackBerry"==b?10<=d.d:"Android"==b?2<d.d||2==d.d&&1<d.o:526<=g.d||525<=g.d&&13<=g.o,new D(f,h,k,"AppleWebKit",g,e,b,d,c,I(a.I),new A(p,536>g.d||536==g.d&&11>g.o,"iPhone"==b||"iPad"==b||"iPod"==b||"Macintosh"==b))}function H(a,b,c){return(a=a.match(b))&&a[c]?a[c]:""}function I(a){return a.documentMode?a.documentMode:void 0}function ia(a){this.ua=a||"-"}function J(a,b){this.K=a,this.V=4,this.L="n";var c=(b||"n4").match(/^([nio])([1-9])$/i);c&&(this.L=c[1],this.V=parseInt(c[2],10))}function K(a){return a.L+a.V}function ja(a){var b=4,c="n",d=l;return a&&((d=a.match(/(normal|oblique|italic)/i))&&d[1]&&(c=d[1].substr(0,1).toLowerCase()),(d=a.match(/([1-9]00|normal|bold)/i))&&d[1]&&(/bold/i.test(d[1])?b=7:/[1-9]00/.test(d[1])&&(b=parseInt(d[1].substr(0,1),10)))),c+b}function ka(a,b,c){this.c=a,this.h=b,this.O=c,this.j="wf",this.g=new ia("-")}function L(a){x(a.h,a.g.f(a.j,"loading")),da(a.h,a.g.f(a.j,"active"))||w(a.h,a.g.f(a.j,"inactive")),M(a,"inactive")}function M(a,b,c){a.O[b]&&(c?a.O[b](c.getName(),K(c)):a.O[b]())}function la(){this.w={}}function O(a,b){this.c=a,this.C=b,this.s=this.c.createElement("span",{"aria-hidden":"true"},this.C)}function P(a,b){var c;c=[];for(var d=b.K.split(/,\s*/),e=0;e<d.length;e++){var g=d[e].replace(/['"]/g,"");c.push(-1==g.indexOf(" ")?g:"'"+g+"'")}c=c.join(","),d="normal",e=b.V+"00","o"===b.L?d="oblique":"i"===b.L&&(d="italic"),a.s.style.cssText="position:absolute;top:-999px;left:-999px;font-size:300px;width:auto;height:auto;line-height:normal;margin:0;padding:0;font-variant:normal;white-space:nowrap;font-family:"+c+";"+("font-style:"+d+";font-weight:"+e+";")}function Q(a){ca(a.c,"body",a.s)}function ma(a,b,c,d,e,g,f,h){this.W=a,this.sa=b,this.c=c,this.q=d,this.C=h||"BESbswy",this.k=e,this.F={},this.T=g||5e3,this.Z=f||l,this.B=this.A=l,a=new O(this.c,this.C),Q(a);for(var k in R)R.hasOwnProperty(k)&&(P(a,new J(R[k],K(this.q))),this.F[R[k]]=a.s.offsetWidth);a.remove()}function pa(a,b,c){for(var d in R)if(R.hasOwnProperty(d)&&b===a.F[R[d]]&&c===a.F[R[d]])return j;return m}function oa(a){var b=a.A.s.offsetWidth,c=a.B.s.offsetWidth;b===a.F.serif&&c===a.F["sans-serif"]||a.k.U&&pa(a,b,c)?u()-a.xa>=a.T?a.k.U&&pa(a,b,c)&&(a.Z===l||a.Z.hasOwnProperty(a.q.getName()))?S(a,a.W):S(a,a.sa):setTimeout(t(function(){oa(this)},a),25):S(a,a.W)}function S(a,b){a.A.remove(),a.B.remove(),b(a.q)}function T(a,b,c,d){this.c=b,this.t=c,this.P=0,this.ba=this.Y=m,this.T=d,this.k=a.k}function qa(a,b,c,d,e){if(0===b.length&&e)L(a.t);else for(a.P+=b.length,e&&(a.Y=e),e=0;e<b.length;e++){var g=b[e],f=c[g.getName()],h=a.t,k=g;
w(h.h,h.g.f(h.j,k.getName(),K(k).toString(),"loading")),M(h,"fontloading",k),new ma(t(a.ga,a),t(a.ha,a),a.c,g,a.k,a.T,d,f).start()}}function ra(a){0==--a.P&&a.Y&&(a.ba?(a=a.t,x(a.h,a.g.f(a.j,"loading")),x(a.h,a.g.f(a.j,"inactive")),w(a.h,a.g.f(a.j,"active")),M(a,"active")):L(a.t))}function U(a){this.G=a,this.u=new la,this.ya=new E(a.navigator.userAgent,a.document),this.a=this.ya.parse(),this.Q=this.R=0}function sa(a,b,c){this.N=a?a:b+ta,this.p=[],this.S=[],this.ca=c||""}function ua(a){this.p=a,this.$=[],this.J={}}function V(a,b){this.a=new E(navigator.userAgent,document).parse(),this.c=a,this.e=b}function W(a,b){this.c=a,this.e=b,this.m=[]}function X(a,b){this.c=a,this.e=b,this.m=[]}function Y(a,b){this.c=a,this.e=b,this.m=[]}function Z(a,b){this.c=a,this.e=b}var j=!0,l=null,m=!1,q=this,u=Date.now||function(){return+new Date};v.prototype.createElement=function(a,b,c){if(a=this.z.createElement(a),b)for(var d in b)b.hasOwnProperty(d)&&("style"==d?a.style.cssText=b[d]:a.setAttribute(d,b[d]));return c&&a.appendChild(this.z.createTextNode(c)),a},r("webfont.BrowserInfo",A),A.prototype.pa=n("M"),A.prototype.hasWebFontSupport=A.prototype.pa,A.prototype.qa=n("U"),A.prototype.hasWebKitFallbackBug=A.prototype.qa,A.prototype.ra=n("Aa"),A.prototype.hasWebKitMetricsBug=A.prototype.ra;var fa=/^([0-9]+)(?:[\._-]([0-9]+))?(?:[\._-]([0-9]+))?(?:[\._+-]?(.*))?$/;B.prototype.toString=function(){return[this.d,this.o||"",this.aa||"",this.f||""].join("")},r("webfont.UserAgent",D),D.prototype.getName=n("K"),D.prototype.getName=D.prototype.getName,D.prototype.oa=n("za"),D.prototype.getVersion=D.prototype.oa,D.prototype.ka=n("fa"),D.prototype.getEngine=D.prototype.ka,D.prototype.la=n("ea"),D.prototype.getEngineVersion=D.prototype.la,D.prototype.ma=n("wa"),D.prototype.getPlatform=D.prototype.ma,D.prototype.na=n("va"),D.prototype.getPlatformVersion=D.prototype.na,D.prototype.ja=n("da"),D.prototype.getDocumentMode=D.prototype.ja,D.prototype.ia=n("k"),D.prototype.getBrowserInfo=D.prototype.ia;var ga=new D("Unknown",new B,"Unknown","Unknown",new B,"Unknown","Unknown",new B,"Unknown",void 0,new A(m,m,m));E.prototype.parse=function(){var a;if(-1!=this.a.indexOf("MSIE")||-1!=this.a.indexOf("Trident/")){a=F(this);var b=G(this),c=C(b),d=l,e=l,g=l,f=l,h=H(this.a,/Trident\/([\d\w\.]+)/,1),k=I(this.I),d=-1!=this.a.indexOf("MSIE")?H(this.a,/MSIE ([\d\w\.]+)/,1):H(this.a,/rv:([\d\w\.]+)/,1),e=C(d);""!=h?(g="Trident",f=C(h)):(g="Unknown",f=new B,h="Unknown"),a=new D("MSIE",e,d,g,f,h,a,c,b,k,new A("Windows"==a&&6<=e.d||"Windows Phone"==a&&8<=c.d,m,m))}else if(-1!=this.a.indexOf("Opera"))a:if(a="Unknown",b=H(this.a,/Presto\/([\d\w\.]+)/,1),c=C(b),d=G(this),e=C(d),g=I(this.I),c.d!==l?a="Presto":(-1!=this.a.indexOf("Gecko")&&(a="Gecko"),b=H(this.a,/rv:([^\)]+)/,1),c=C(b)),-1!=this.a.indexOf("Opera Mini/"))f=H(this.a,/Opera Mini\/([\d\.]+)/,1),h=C(f),a=new D("OperaMini",h,f,a,c,b,F(this),e,d,g,new A(m,m,m));else{if(-1!=this.a.indexOf("Version/")&&(f=H(this.a,/Version\/([\d\.]+)/,1),h=C(f),h.d!==l)){a=new D("Opera",h,f,a,c,b,F(this),e,d,g,new A(10<=h.d,m,m));break a}f=H(this.a,/Opera[\/ ]([\d\.]+)/,1),h=C(f),a=h.d!==l?new D("Opera",h,f,a,c,b,F(this),e,d,g,new A(10<=h.d,m,m)):new D("Opera",new B,"Unknown",a,c,b,F(this),e,d,g,new A(m,m,m))}else/OPR\/[\d.]+/.test(this.a)?a=ha(this):/AppleWeb(K|k)it/.test(this.a)?a=ha(this):-1!=this.a.indexOf("Gecko")?(a="Unknown",b=new B,c="Unknown",d=G(this),e=C(d),g=m,-1!=this.a.indexOf("Firefox")?(a="Firefox",c=H(this.a,/Firefox\/([\d\w\.]+)/,1),b=C(c),g=3<=b.d&&5<=b.o):-1!=this.a.indexOf("Mozilla")&&(a="Mozilla"),f=H(this.a,/rv:([^\)]+)/,1),h=C(f),g||(g=1<h.d||1==h.d&&9<h.o||1==h.d&&9==h.o&&2<=h.aa||f.match(/1\.9\.1b[123]/)!=l||f.match(/1\.9\.1\.[\d\.]+/)!=l),a=new D(a,b,c,"Gecko",h,f,F(this),e,d,I(this.I),new A(g,m,m))):a=ga;return a},ia.prototype.f=function(){for(var b=[],c=0;c<arguments.length;c++)b.push(arguments[c].replace(/[\W_]+/g,"").toLowerCase());return b.join(this.ua)},J.prototype.getName=n("K"),O.prototype.remove=function(){var a=this.s;a.parentNode&&a.parentNode.removeChild(a)};var R={Da:"serif",Ca:"sans-serif",Ba:"monospace"};ma.prototype.start=function(){this.A=new O(this.c,this.C),Q(this.A),this.B=new O(this.c,this.C),Q(this.B),this.xa=u(),P(this.A,new J(this.q.getName()+",serif",K(this.q))),P(this.B,new J(this.q.getName()+",sans-serif",K(this.q))),oa(this)},T.prototype.ga=function(a){var b=this.t;x(b.h,b.g.f(b.j,a.getName(),K(a).toString(),"loading")),x(b.h,b.g.f(b.j,a.getName(),K(a).toString(),"inactive")),w(b.h,b.g.f(b.j,a.getName(),K(a).toString(),"active")),M(b,"fontactive",a),this.ba=j,ra(this)},T.prototype.ha=function(a){var b=this.t;x(b.h,b.g.f(b.j,a.getName(),K(a).toString(),"loading")),da(b.h,b.g.f(b.j,a.getName(),K(a).toString(),"active"))||w(b.h,b.g.f(b.j,a.getName(),K(a).toString(),"inactive")),M(b,"fontinactive",a),ra(this)},U.prototype.load=function(a){var b=a.context||this.G;this.c=new v(this.G,b);var b=new ka(this.c,b.document.documentElement,a),c=[],d=a.timeout;w(b.h,b.g.f(b.j,"loading")),M(b,"loading");var f,c=this.u,e=this.c,g=[];for(f in a)if(a.hasOwnProperty(f)){var h=c.w[f];h&&g.push(h(a[f],e))}for(c=g,this.Q=this.R=c.length,a=new T(this.a,this.c,b,d),f=0,d=c.length;d>f;f++)e=c[f],e.H(this.a,t(this.ta,this,e,b,a))},U.prototype.ta=function(a,b,c,d){var e=this;d?a.load(function(a,b,d){var k=0==--e.R;setTimeout(function(){qa(c,a,b||{},d||l,k)},0)}):(a=0==--this.R,this.Q--,a&&0==this.Q&&L(b),qa(c,[],{},l,a))};var ta="//fonts.googleapis.com/css";sa.prototype.f=function(){if(0==this.p.length)throw Error("No fonts to load!");if(-1!=this.N.indexOf("kit="))return this.N;for(var a=this.p.length,b=[],c=0;a>c;c++)b.push(this.p[c].replace(/ /g,"+"));return a=this.N+"?family="+b.join("%7C"),0<this.S.length&&(a+="&subset="+this.S.join(",")),0<this.ca.length&&(a+="&text="+encodeURIComponent(this.ca)),a};var va={latin:"BESbswy",cyrillic:"&#1081;&#1103;&#1046;",greek:"&#945;&#946;&#931;",khmer:"&#x1780;&#x1781;&#x1782;",Hanuman:"&#x1780;&#x1781;&#x1782;"},wa={thin:"1",extralight:"2","extra-light":"2",ultralight:"2","ultra-light":"2",light:"3",regular:"4",book:"4",medium:"5","semi-bold":"6",semibold:"6","demi-bold":"6",demibold:"6",bold:"7","extra-bold":"8",extrabold:"8","ultra-bold":"8",ultrabold:"8",black:"9",heavy:"9",l:"3",r:"4",b:"7"},xa={i:"i",italic:"i",n:"n",normal:"n"},ya=RegExp("^(thin|(?:(?:extra|ultra)-?)?light|regular|book|medium|(?:(?:semi|demi|extra|ultra)-?)?bold|black|heavy|l|r|b|[1-9]00)?(n|i|normal|italic)?$");ua.prototype.parse=function(){for(var a=this.p.length,b=0;a>b;b++){var c=this.p[b].split(":"),d=c[0].replace(/\+/g," "),e=["n4"];if(2<=c.length){var g,f=c[1];if(g=[],f)for(var f=f.split(","),h=f.length,k=0;h>k;k++){var p;if(p=f[k],p.match(/^[\w-]+$/)){p=ya.exec(p.toLowerCase());var s=void 0;if(p==l)s="";else{if(s=void 0,s=p[1],s==l||""==s)s="4";else var na=wa[s],s=na?na:isNaN(s)?"4":s.substr(0,1);s=[p[2]==l||""==p[2]?"n":xa[p[2]],s].join("")}p=s}else p="";p&&g.push(p)}0<g.length&&(e=g),3==c.length&&(c=c[2],g=[],c=c?c.split(","):g,0<c.length&&(c=va[c[0]])&&(this.J[d]=c))}for(this.J[d]||(c=va[d])&&(this.J[d]=c),c=0;c<e.length;c+=1)this.$.push(new J(d,e[c]))}};var za={Arimo:j,Cousine:j,Tinos:j};V.prototype.H=function(a,b){b(a.k.M)},V.prototype.load=function(a){var b=this.c;if("MSIE"==this.a.getName()&&this.e.blocking!=j){var c=t(this.X,this,a),d=function(){b.z.body?c():setTimeout(d,0)};d()}else this.X(a)},V.prototype.X=function(a){for(var b=this.c,c=new sa(this.e.api,y(b),this.e.text),d=this.e.families,e=d.length,g=0;e>g;g++){var f=d[g].split(":");3==f.length&&c.S.push(f.pop());var h="";2==f.length&&""!=f[1]&&(h=":"),c.p.push(f.join(h))}d=new ua(d),d.parse(),ea(b,c.f()),a(d.$,d.J,za)},W.prototype.D=function(a){return y(this.c)+(this.e.api||"//f.fontdeck.com/s/css/js/")+(this.c.v.location.hostname||this.c.G.location.hostname)+"/"+a+".js"},W.prototype.H=function(a,b){var c=this.e.id,d=this.c.v,e=this;c?(d.__webfontfontdeckmodule__||(d.__webfontfontdeckmodule__={}),d.__webfontfontdeckmodule__[c]=function(a,c){for(var d=0,k=c.fonts.length;k>d;++d){var p=c.fonts[d];e.m.push(new J(p.name,ja("font-weight:"+p.weight+";font-style:"+p.style)))}b(a)},z(this.c,this.D(c),function(a){a&&b(m)})):b(m)},W.prototype.load=function(a){a(this.m)},X.prototype.D=function(a){var b=y(this.c);return(this.e.api||b+"//use.typekit.net")+"/"+a+".js"},X.prototype.H=function(a,b){var c=this.e.id,d=this.e,e=this.c.v,g=this;c?(e.__webfonttypekitmodule__||(e.__webfonttypekitmodule__={}),e.__webfonttypekitmodule__[c]=function(c){c(a,d,function(a,c,d){for(var e=0;e<c.length;e+=1){var f=d[c[e]];if(f)for(var N=0;N<f.length;N+=1)g.m.push(new J(c[e],f[N]));else g.m.push(new J(c[e]))}b(a)})},z(this.c,this.D(c),function(a){a&&b(m)},2e3)):b(m)},X.prototype.load=function(a){a(this.m)},Y.prototype.H=function(a,b){var c=this,d=c.e.projectId,e=c.e.version;if(d){var g=c.c.v;z(this.c,c.D(d,e),function(e){if(e)b(m);else{if(g["__mti_fntLst"+d]&&(e=g["__mti_fntLst"+d]()))for(var h=0;h<e.length;h++)c.m.push(new J(e[h].fontfamily));b(a.k.M)}}).id="__MonotypeAPIScript__"+d}else b(m)},Y.prototype.D=function(a,b){var c=y(this.c),d=(this.e.api||"fast.fonts.net/jsapi").replace(/^.*http(s?):(\/\/)?/,"");return c+"//"+d+"/"+a+".js"+(b?"?v="+b:"")},Y.prototype.load=function(a){a(this.m)},Z.prototype.load=function(a){var b,c,d=this.e.urls||[],e=this.e.families||[],g=this.e.testStrings||{};for(b=0,c=d.length;c>b;b++)ea(this.c,d[b]);for(d=[],b=0,c=e.length;c>b;b++){var f=e[b].split(":");if(f[1])for(var h=f[1].split(","),k=0;k<h.length;k+=1)d.push(new J(f[0],h[k]));else d.push(new J(f[0]))}a(d,g)},Z.prototype.H=function(a,b){return b(a.k.M)};var $=new U(q);$.u.w.custom=function(a,b){return new Z(b,a)},$.u.w.fontdeck=function(a,b){return new W(b,a)},$.u.w.monotype=function(a,b){return new Y(b,a)},$.u.w.typekit=function(a,b){return new X(b,a)},$.u.w.google=function(a,b){return new V(b,a)},q.WebFont||(q.WebFont={},q.WebFont.load=t($.load,$),q.WebFontConfig&&$.load(q.WebFontConfig))}(this,document),function(v){"use awesome";try{var k=function(){function k(b){b=b||navigator.userAgent;var d=b.match(/Trident\/([\d.]+)/);return d&&"7.0"===d[1]?11:(b=b.match(/MSIE ([\d.]+)/))?parseInt(b[1],10):!1}return{slice:Array.prototype.slice,uuid:function(){return"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(b){var d=16*Math.random()|0;return("x"==b?d:3&d|8).toString(16)})},reduce:function(b){try{return"object"==typeof b||"function"==typeof b?b.toString():b}catch(d){return"unknown"}},defer:function(b,d){setTimeout(function(){b.apply(d)})},isoNow:function(){function b(a){return a=String(a),1===a.length&&(a="0"+a),a}var d=new Date;return d.getUTCFullYear()+"-"+b(d.getUTCMonth()+1)+"-"+b(d.getUTCDate())+"T"+b(d.getUTCHours())+":"+b(d.getUTCMinutes())+":"+b(d.getUTCSeconds())+"."+String((d.getUTCMilliseconds()/1e3).toFixed(3)).slice(2,5)+"Z"},isBrowserIE:k,isBrowserSupported:function(b){return b=b||navigator.userAgent,b=k(b),!b||b>8},contains:function(b,d){var a;for(a=0;a<b.length;a++)if(b[a]===d)return!0;return!1}}}(this),n=function(){var f={endpoint:"https://my.trackjs.com/capture",cdnHost:"dl1d2m8ri9v3j.cloudfront.net",version:"1.2.5.0",trackGlobal:!0,trackAjaxFail:!0,trackConsoleError:!0,inspectors:!0,consoleDisplay:!0,globalAlias:!0,userId:void 0,sessionId:void 0,ignore:[],mergeCustomerConfig:function(b){if(b){var a,e,d="userId sessionId trackGlobal trackAjaxFail trackAjaxFail trackConsoleError inspectors consoleDisplay globalAlias ignore".split(" ");for(a=0;a<d.length;a++)e=d[a],void 0!==b[e]&&(f[e]=b[e])}},initialize:function(){v._trackJs&&f.mergeCustomerConfig(v._trackJs),k.isBrowserIE()&&(f.endpoint="//"+f.endpoint.split("://")[1])}};return f}(this),m=function(e){function f(g,c){h[g]||(h[g]=[]);var a=k.uuid();return h[g].push({key:a,value:c}),10<h[g].length&&(h[g]=h[g].slice(Math.max(h[g].length-10,0))),a}function b(g,c,a){function b(g,c){var a=new e.XMLHttpRequest;return"withCredentials"in a?(a.open(g,c),a.setRequestHeader("Content-Type","text/plain")):"undefined"!=typeof e.XDomainRequest?(a=new e.XDomainRequest,a.open(g,c)):a=null,a}try{if(!m){var l=b(g,c);l.onreadystatechange=function(){4===l.readyState&&200!==l.status&&(m=!0)},l.tjs=void 0,l.send(JSON.stringify(a))}}catch(d){m=!0}}function d(){var g=(new Date).getTime();if(w++,r+1e3>=g){if(r=g,w>10)return s++,!0}else w=0,r=g;return!1}function a(){var g=s;return s=0,g}function p(g,c,l,h,q,p){g={column:q,entry:g,file:l,line:h,url:e.location.toString(),message:k.reduce(c),stack:p,timestamp:k.isoNow()};for(var f in t)t.hasOwnProperty(f)&&(c=t[f],"function"==typeof c.onTransmit&&(g[f]=c.onTransmit()));if(!d()){g.throttled=a();a:{for(f=0;f<n.ignore.length;f++)if(n.ignore[f]&&n.ignore[f].test&&n.ignore[f].test(g.message)){f=!0;break a}f=!1}f||b("POST",n.endpoint,g)}}function c(g){var a,c=k.slice.call(arguments,1);for(a in g)"function"==typeof g[a]&&(k.contains(c,a)||function(){var c=g[a];g[a]=function(){try{var g=k.slice.call(arguments,0);return c.apply(this,g)}catch(a){throw q("catch",a),a}}}())}function l(g){for(var a in g)if(g.hasOwnProperty(a)){var c=g[a];"function"==typeof c.onInitialize&&c.onInitialize()}}function q(g,a){p(g,a.message,a.fileName,a.lineNumber,void 0,a.stack)}var t={},h={},m=!1,w=0,s=0,r=(new Date).getTime();return{registerModule:function(a,c){return a?(t[a]={onInitialize:c.onInitialize,onTransmit:c.onTransmit,forTest:c.forTest},!0):!1},getModule:function(a){return t.hasOwnProperty(a)?t[a]:!1},addLogEntry:f,getLogEntry:function(a,c){h[a]||(h[a]=[]);for(var b=0;b<h[a].length;b++)if(h[a][b].key===c)return h[a][b].value;return!1},flushLog:function(a){h[a]||(h[a]=[]);for(var c=[],b=0;b<h[a].length;b++)c.push(h[a][b].value);return h[a].length=0,c},updateLogEntry:function(a,c,b){h[a]||(h[a]=[]);for(var l=0;l<h[a].length;l++)if(h[a][l].key===c)return h[a][l].value=b,!0;return!1},transmit:p,transmitErrorObject:q,initialize:function(){n.initialize(),l(t),n.trackGlobal&&n.inspectors&&(e.onerror=function(a,c,b,l,g){g=g||{},p("global",a,c,b,l,g.stack)}),e.trackJs={track:function(a){"[object Error]"===Object.prototype.toString.call(a)?q("direct",a):p("direct",a)},attempt:function(a,c){try{var b=k.slice.call(arguments,2);return a.apply(c||this,b)}catch(l){throw q("catch",l),l}},watch:function(a,c){return function(){try{var b=k.slice.call(arguments,0);return a.apply(c||this,b)}catch(l){throw q("catch",l),l}}},watchAll:c,trackAll:c,configure:n.mergeCustomerConfig,version:n.version};var a,b=["log","debug","info","warn","error"];for(a=0;a<b.length;a++)(function(a){e.trackJs[a]=function(){var c=k.slice.call(arguments);f("c",{timestamp:k.isoNow(),severity:a,message:k.reduce(c)}),"error"===a&&n.trackConsoleError&&("[object Error]"===Object.prototype.toString.call(c[0])?q("console",c[0]):p("console",k.reduce(c)))}})(b[a]);n.globalAlias&&(e.track=e.trackJs.track)},forTest:{initializeModules:l,throttle:d,getThrottledCount:a,wrap:c}}}(this);!function(e){function p(a,c){return this.tjs={method:a[0],url:a[1]},c.apply(this,a)}function c(a,c){function b(a){if(a.tjs){var c=m.getLogEntry("n",a.tjs.logId);c&&(c.completedOn=k.isoNow(),c.statusCode=1223==a.status?204:a.status,c.statusText=1223==a.status?"No Content":a.statusText,m.updateLogEntry("n",a.tjs.logId,c),a.tjs=void 0)}}function d(a){n.trackAjaxFail&&400<=a.status&&1223!=a.status&&m.transmit("ajax",a.status+" "+a.statusText)}if(!this.tjs)return c.apply(this,a);if(this.tjs.logId=m.addLogEntry("n",{startedOn:k.isoNow(),method:this.tjs.method,url:this.tjs.url}),e.ProgressEvent&&this.addEventListener&&this.addEventListener("readystatechange",function(){4===this.readyState&&b(this)},!0),this.addEventListener)this.addEventListener("load",function(){b(this),d(this)},!0);else if("[object XDomainRequest]"===this.toString()){var f=this.onload;this.onload=function(){b(this),"function"==typeof f&&f.apply(this,arguments)};var p=this.onerror;this.onerror=function(){b(this),"function"==typeof p&&p.apply(this,arguments)}}else{var s=this.onreadystatechange,r=function(){4===this.readyState&&(b(this),d(this)),"function"==typeof s&&s.apply(this,arguments)};this.onreadystatechange=r,k.defer(function(){this.onreadystatechange!==r&&(s=this.onreadystatechange,this.onreadystatechange=r)},this)}return c.apply(this,a)}var f,b,d,a;m.registerModule("network",{onInitialize:function(){k.isBrowserSupported()&&n.inspectors&&(d=e.XMLHttpRequest.prototype.open,a=e.XMLHttpRequest.prototype.send,e.XMLHttpRequest.prototype.open=function(){var a=k.slice.call(arguments,0);return p.call(this,a,d)},e.XMLHttpRequest.prototype.send=function(){var b=k.slice.call(arguments,0);return c.call(this,b,a)},e.XDomainRequest&&(f=e.XDomainRequest.prototype.open,b=e.XDomainRequest.prototype.send,e.XDomainRequest.prototype.open=function(){var a=k.slice.call(arguments,0);return p.call(this,a,f)},e.XDomainRequest.prototype.send=function(){var a=k.slice.call(arguments,0);return c.call(this,a,b)}))},onTransmit:function(){return m.flushLog("n")}})}(this),function(){function f(a,b,d){for(var f={},h=a.attributes,e=0;e<h.length;e++)"value"!==h[e].name.toLowerCase()&&(f[h[e].name]=h[e].value);return h=a.getBoundingClientRect(),{tag:a.tagName.toLowerCase(),attributes:f,position:{left:h.left,top:h.top,width:h.width,height:h.height},value:b?{length:b.length,pattern:""===b||void 0===b?"empty":/^[a-z0-9!#$%&'*+=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(b)?"email":/^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/.test(b)||/^(\d{4}[\/\-](0?[1-9]|1[012])[\/\-]0?[1-9]|[12][0-9]|3[01])$/.test(b)?"date":/^(?:(?:\+?1\s*(?:[.-]\s*)?)?(?:\(\s*([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9])\s*\)|([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9]))\s*(?:[.-]\s*)?)?([2-9]1[02-9]|[2-9][02-9]1|[2-9][02-9]{2})\s*(?:[.-]\s*)?([0-9]{4})(?:\s*(?:#|x\.?|ext\.?|extension)\s*(\d+))?$/.test(b)?"usphone":/^\s*$/.test(b)?"whitespace":/^\d*$/.test(b)?"numeric":/^[a-zA-Z]*$/.test(b)?"alpha":/^[a-zA-Z0-9]*$/.test(b)?"alphanumeric":"characters",checked:d}:void 0}}function b(a,b,d){if(a.tagName.toLowerCase()!==b.toLowerCase())return!1;if(!d)return!0;for(a=(a.getAttribute("type")||"").toLowerCase(),b=0;b<d.length;b++)if(d[b]===a)return!0;return!1}function d(a,b,d,e){m.addLogEntry("v",{timestamp:k.isoNow(),action:b,element:f(a,d,e)})}function a(a){(a=a.target||document.elementFromPoint(a.clientX,a.clientY))&&a.tagName&&(b(a,"input",["checkbox"])&&d(a,"input",a.value,a.checked),b(a,"input",["radio"])&&d(a,"input",a.value,a.checked),(b(a,"a")||b(a,"button")||b(a,"input",["button","submit"]))&&d(a,"click"))}function p(a){if((a=a.target||document.elementFromPoint(a.clientX,a.clientY))&&a.tagName&&(b(a,"textarea")&&d(a,"input",a.value),b(a,"select")&&a.options&&0<a.options.length&&null!=a.options[a.selectedIndex]&&d(a,"input",a.options[a.selectedIndex].value),b(a,"input")&&!b(a,"input",["button","submit","hidden","checkbox","radio"]))){var e=(a.getAttribute("type")||"").toLowerCase();d(a,"input","password"===e?void 0:a.value)}}m.registerModule("visitor",{onInitialize:function(){n.inspectors&&(document.addEventListener?(document.addEventListener("click",a,!0),document.addEventListener("blur",p,!0)):document.attachEvent&&(document.attachEvent("onclick",a),document.attachEvent("onfocusout",p)))},onTransmit:function(){return m.flushLog("v")},forTest:{onDocumentClicked:a,onInputChanged:p}})}(this),function(e){function f(){if(e._trackJs&&e._trackJs.customer)return e._trackJs.customer;var a=document.getElementsByTagName("script");return a[a.length-1].getAttribute("data-customer")}function b(a){if(a.token){var b=new Image;setTimeout(function(){b.src="//"+n.cdnHost+"/usage.gif?customer="+a.token+"&correlationId="+a.correlationId+"&x="+k.uuid()},0)}}var d={};m.registerModule("customer",{onInitialize:function(){d.token=f();var a=document.cookie.replace(/(?:(?:^|.*;\s*)TJS\s*\=\s*([^;]*).*$)|^.*$/,"$1");a||(a=k.uuid(),document.cookie="TJS="+a+"; expires=Fri, 31 Dec 9999 23:59:59 GMT; path=/"),d.correlationId=a,b(d)},onTransmit:function(){return d.userId=n.userId,d.sessionId=n.sessionId,d},forTest:{getCustomerToken:f}})}(this),function(e){function f(b,d){var f,a=b||{},e=a.log||function(){},c=["log","debug","info","warn","error"];for(f=0;f<c.length;f++)(function(b){var c=a[b]||e;a[b]=function(){var a=k.slice.call(arguments);m.addLogEntry("c",{timestamp:k.isoNow(),severity:b,message:k.reduce(a)}),"error"===b&&d.trackConsoleError&&("[object Error]"===Object.prototype.toString.call(a[0])?m.transmitErrorObject("console",a[0]):m.transmit("console",k.reduce(a))),d.consoleDisplay&&(c.apply?c.apply(this,a):c(a))}})(c[f]);return a}m.registerModule("console",{onInitialize:function(){n.inspectors&&(e.console=f(e.console,n))},onTransmit:function(){return m.flushLog("c")},forTest:{listenToConsole:f}})}(this),function(e,f){function b(a){var b,c={};a.jQuery&&a.jQuery.fn&&a.jQuery.fn.jquery&&(c.jQuery=a.jQuery.fn.jquery),a.jQuery&&a.jQuery.ui&&a.jQuery.ui.version&&(c.jQueryUI=a.jQuery.ui.version),a.angular&&a.angular.version&&a.angular.version.full&&(c.angular=a.angular.version.full);for(b in a)if("webkitStorageInfo"!==b)try{if(a[b]){var d=a[b].version||a[b].Version||a[b].VERSION;"string"==typeof d&&(c[b]=d)}}catch(e){}return c}var d=(new Date).getTime();m.registerModule("environment",{onTransmit:function(){return{userAgent:f.navigator.userAgent,age:(new Date).getTime()-d,viewportHeight:document.documentElement.clientHeight,viewportWidth:document.documentElement.clientWidth,dependencies:b(f)}},forTest:{discoverDependencies:b}})}(this,v),m.initialize()}catch(u){m.transmit("tracker",u.message,u.fileName,u.lineNumber,void 0,u.stack)}}(window);